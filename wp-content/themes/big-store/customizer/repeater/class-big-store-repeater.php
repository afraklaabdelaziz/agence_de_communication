...
/**
 * Class Big_Store_Repeater
 */
class Big_Store_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'big-store' );
...
//number control//////////
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'big-store' ); ?>
					</button>
				</div>
			</div>
			<?php
	...
	private function image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Image', 'big-store' ); ?>
			</span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'big-store' ); ?>" />
		</div>
		<?php
	}
	...
	private function logo_image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Logo Image', 'big-store' ); ?>
			</span>
			<input type="text" class="widefat custom-logo-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-logo-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'big-store' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice( $value = 'customizer_repeater_icon' ) {
		?>
		<span class="customize-control-title">
			<?php esc_html_e( 'Image type', 'big-store' ); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected( $value, 'customizer_repeater_icon' ); ?>><?php esc_html_e( 'Icon', 'big-store' ); ?></option>
			<option value="customizer_repeater_image" <?php selected( $value, 'customizer_repeater_image' ); ?>><?php esc_html_e( 'Image', 'big-store' ); ?></option>
			<option value="customizer_repeater_logo_image" <?php selected( $value, 'customizer_repeater_logo_image' ); ?>><?php esc_html_e( 'Logo Image', 'big-store' ); ?></option>
			<option value="customizer_repeater_none" <?php selected( $value, 'customizer_repeater_none' ); ?>><?php esc_html_e( 'None', 'big-store' ); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'big-store' ); ?></span>
		<?php
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<?php get_template_part( $this->customizer_icon_container ); ?>
					<input type="text" class="customizer-repeater-social-repeater-link" placeholder="<?php esc_attr_e( 'Link', 'big-store' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'big-store' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'big-store' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					if ( ! empty( $social_icon['icon'] ) ) {
						...
						echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_attr__( 'Link', 'big-store' ) . '" value="';
					...
					if ( $show_del == 1 ) {
						...
						echo '">' . esc_html__( 'Remove Icon', 'big-store' ) . '</button>';
					echo '</div>';
				}
				?>
