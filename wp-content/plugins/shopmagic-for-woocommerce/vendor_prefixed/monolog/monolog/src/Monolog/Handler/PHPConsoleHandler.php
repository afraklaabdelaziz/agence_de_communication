...
use ShopMagicVendor\PhpConsole\Connector;
use ShopMagicVendor\PhpConsole\Handler;
use ShopMagicVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \ShopMagicVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \ShopMagicVendor\PhpConsole\Connector $connector = null, $level = \ShopMagicVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('ShopMagicVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\ShopMagicVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \ShopMagicVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \ShopMagicVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\ShopMagicVendor\PhpConsole\Helper::isRegistered()) {
            \ShopMagicVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \ShopMagicVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \ShopMagicVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \ShopMagicVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \ShopMagicVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
