...
namespace ShopMagicVendor\Pelago;
...
class Emogrifier
{
     ...
     * @var bool
     */
    private $shouldRemoveInvisibleNodes = \true;
    /**
    ...
    private function parseCssRules($css)
    {
        ...
        if (!isset($this->caches[static::CACHE_KEY_CSS][$cssKey])) {
            ...
            $cssRules = ['inlineable' => [], 'uninlineable' => []];
            /** @var string[][] $matches */
            /** @var string[] $cssRule */
    ...
    {
        $nodesWithStyleDisplayNone = $this->xPath->query('//*[contains(translate(translate(@style," ",""),"NOE","noe"),"display:none")]');
    ...
    {
        $normalizedOriginalStyle = \preg_replace_callback('/[A-z\\-]+(?=\\:)/S', static function (array $m) {
    ...
    {
        $cssRulesRelevantForDocument = \array_filter($cssRules, function (array $cssRule) {
        ...
        if (!\class_exists('ShopMagicVendor\\Pelago\\Emogrifier\\CssConcatenator')) {
            require_once __DIR__ . '/Emogrifier/CssConcatenator.php';
        }
        $cssConcatenator = new \ShopMagicVendor\Pelago\Emogrifier\CssConcatenator();
    ...
    {
        return 'contains(concat(" ",@class," "),concat(" ","' . \implode('"," "))][contains(concat(" ",@class," "),concat(" ","', \explode('.', \substr($match[2], 1))) . '"," "))';
    }
    /**
     * @param string[] $match
     *
    ...
    private function translateNthChild(array $match)
    {
        ...
        if (isset($parseResult[static::MULTIPLIER])) {
            if ($parseResult[static::MULTIPLIER] < 0) {
                ...
                $xPathExpression = \sprintf('*[(last() - position()) mod %1%u = %2$u]/self::%3$s', $parseResult[static::MULTIPLIER], $parseResult[static::INDEX], $match[1]);
            } else {
                $xPathExpression = \sprintf('*[position() mod %1$u = %2$u]/self::%3$s', $parseResult[static::MULTIPLIER], $parseResult[static::INDEX], $match[1]);
            }
        } else {
    ...
    private function translateNthOfType(array $match)
    {
        ...
        if (isset($parseResult[static::MULTIPLIER])) {
            if ($parseResult[static::MULTIPLIER] < 0) {
                $parseResult[static::MULTIPLIER] = \abs($parseResult[static::MULTIPLIER]);
                $xPathExpression = \sprintf('%1$s[(last() - position()) mod %2$u = %3$u]', $match[1], $parseResult[static::MULTIPLIER], $parseResult[static::INDEX]);
            } else {
                $xPathExpression = \sprintf('%1$s[position() mod %2$u = %3$u]', $match[1], $parseResult[static::MULTIPLIER], $parseResult[static::INDEX]);
            }
        } else {
