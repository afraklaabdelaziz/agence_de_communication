...
use ShopMagicVendor\Symfony\Component\CssSelector\Exception\InternalErrorException;
 ...
 * @internal
 */
class StringHandler implements \ShopMagicVendor\Symfony\Component\CssSelector\Parser\Handler\HandlerInterface
{
    private $patterns;
    private $escaping;
    ...
    public function handle(\ShopMagicVendor\Symfony\Component\CssSelector\Parser\Reader $reader, \ShopMagicVendor\Symfony\Component\CssSelector\Parser\TokenStream $stream)
    {
        ...
        if (!$match) {
            throw new \ShopMagicVendor\Symfony\Component\CssSelector\Exception\InternalErrorException(\sprintf('Should have found at least an empty match at %d.', $reader->getPosition()));
        }
        // check unclosed strings
        if (\strlen($match[0]) === $reader->getRemainingLength()) {
            throw \ShopMagicVendor\Symfony\Component\CssSelector\Exception\SyntaxErrorException::unclosedString($reader->getPosition() - 1);
        }
        ...
        $stream->push(new \ShopMagicVendor\Symfony\Component\CssSelector\Parser\Token(\ShopMagicVendor\Symfony\Component\CssSelector\Parser\Token::TYPE_STRING, $string, $reader->getPosition()));
