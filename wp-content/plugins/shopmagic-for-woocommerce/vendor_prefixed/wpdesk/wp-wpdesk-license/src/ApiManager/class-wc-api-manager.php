...
if (!\class_exists('ShopMagicVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
         ...
         */
        public $text_domain = 'shopmagic-for-woocommerce';
        /**
         * @var string
         */
        public $plugin_url;
        ...
        {
            $password_management = new \ShopMagicVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \ShopMagicVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \ShopMagicVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function inactive_notice()
        {
            ...
            if (\apply_filters('wpdesk_show_plugin_activation_notice', \true)) {
                (new \ShopMagicVendor\WPDesk\Notice\AjaxHandler())->hooks();
                new \ShopMagicVendor\WPDesk\Notice\PermanentDismissibleNotice(\sprintf(\__('The %1$s%2$s%3$s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %4$sClick here%5$s to activate the API key and the plugin.', $this->text_domain), '<strong>', $this->plugin_title, '</strong>', '<a href="' . \esc_url(\admin_url('plugins.php#' . $this->plugin_dir . '-wpdesk-license')) . '">', '</a>'), "notice-{$this->software_product_id}", \ShopMagicVendor\WPDesk\Notice\PermanentDismissibleNotice::NOTICE_TYPE_WARNING);
            }
        }
        /**
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('ShopMagicVendor\\WP_HTTP_BLOCK_EXTERNAL') && \ShopMagicVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('ShopMagicVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\ShopMagicVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
        ...
        {
            $parser = new \ShopMagicVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \ShopMagicVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \ShopMagicVendor\WPDesk\License\Changelog\Formatter($changes);
