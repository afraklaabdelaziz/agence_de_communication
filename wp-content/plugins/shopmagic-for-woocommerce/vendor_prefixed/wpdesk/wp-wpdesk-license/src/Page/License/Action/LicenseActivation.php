...
use ShopMagicVendor\WPDesk\License\ServerAddressRepository;
 ...
 */
class LicenseActivation extends \ShopMagicVendor\WPDesk\License\Page\License\Action\AbstractAction implements \ShopMagicVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \ShopMagicVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        $this->add_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'shopmagic-for-woocommerce'), 'error');
    }
    /**
    ...
    {
        $this->add_error('activate_text', 'activate_msg', \__('Plugin license activated. ', 'shopmagic-for-woocommerce') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \ShopMagicVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
