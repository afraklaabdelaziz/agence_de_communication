...
use ShopMagicVendor\WPDesk\PluginBuilder\Storage\StorageFactory;
/**
...
trait BuilderTrait
{
     ...
     * @return SlimPlugin
     */
    private function build_plugin(\ShopMagicVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    private function init_register_hooks(\ShopMagicVendor\WPDesk_Plugin_Info $plugin_info, \ShopMagicVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        if ($plugin instanceof \ShopMagicVendor\WPDesk\PluginBuilder\Plugin\Activateable) {
            \register_activation_hook($plugin_info->get_plugin_file_name(), [$plugin, 'activate']);
        }
        if ($plugin instanceof \ShopMagicVendor\WPDesk\PluginBuilder\Plugin\Deactivateable) {
     ...
     * @param SlimPlugin $plugin
     */
    private function store_plugin(\ShopMagicVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        $storageFactory = new \ShopMagicVendor\WPDesk\PluginBuilder\Storage\StorageFactory();
     ...
     * @param SlimPlugin $plugin
     */
    private function init_plugin(\ShopMagicVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
