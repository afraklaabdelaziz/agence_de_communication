...
if (!\class_exists('ShopMagicVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        public function send_payload(array $payload)
        {
            if (\class_exists('ShopMagicVendor\\WPDesk_Logger_Factory')) {
                \ShopMagicVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \ShopMagicVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \ShopMagicVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \ShopMagicVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\ShopMagicVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \ShopMagicVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
