...
class HTMLConverter
{
    ...
    {
        $this->xml_converter = (new \ShopMagicVendor\League\Csv\XMLConverter())->rootElement('table')->recordElement('tr')->fieldElement('td');
    }
    /**
    ...
    protected function appendHeaderSection(string $node_name, array $record, \DOMElement $table)
    {
        ...
        $node = $this->xml_converter->rootElement($node_name)->recordElement('tr')->fieldElement('th')->import([$record], $table->ownerDocument);
