...
namespace ShopMagicVendor\League\Csv\Polyfill;

use Generator;
use ShopMagicVendor\League\Csv\Stream;
use SplFileObject;
use TypeError;
...
final class EmptyEscapeParser
{
    ...
    {
        if ($document instanceof \ShopMagicVendor\League\Csv\Stream || $document instanceof \SplFileObject) {
            return $document;
        }
        throw new \TypeError(\sprintf('%s::parse expects parameter 1 to be a %s or a SplFileObject object, %s given', self::class, \ShopMagicVendor\League\Csv\Stream::class, \is_object($document) ? \get_class($document) : \gettype($document)));
    }
    /**
