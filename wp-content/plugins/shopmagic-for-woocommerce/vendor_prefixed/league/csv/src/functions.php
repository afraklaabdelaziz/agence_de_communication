...
function bom_match(string $str) : string
{
    static $list;
    if (null === $list) {
        $list = (new \ReflectionClass(\ShopMagicVendor\League\Csv\ByteSequence::class))->getConstants();
        \rsort($list);
    }
 ...
 * @return int[]
 */
function delimiter_detect(\ShopMagicVendor\League\Csv\Reader $csv, array $delimiters, int $limit = 1) : array
{
    ...
    $stmt = (new \ShopMagicVendor\League\Csv\Statement())->limit($limit);
    $delimiter_stats = static function (array $stats, string $delimiter) use($csv, $stmt, $record_filter) : array {
        $csv->setDelimiter($delimiter);
        $found_records = \array_filter(\iterator_to_array($stmt->process($csv), \false), $record_filter);
...
use ShopMagicVendor\League\Csv;
if (\PHP_VERSION_ID < 70100 && !\function_exists('\\is_iterable')) {
    ...
    {
        return \ShopMagicVendor\League\Csv\is_iterable($iterable);
    }
}
