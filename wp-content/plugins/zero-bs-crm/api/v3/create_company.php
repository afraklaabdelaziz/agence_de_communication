  ...
  / Breaking Checks
   ====================================================== */
	...
	if (!zeroBSCRM_API_is_zbs_api_authorised()){
		...
		$we_have_tags = false; $id = -1; $email = ''; $fieldPrefix = 'zbsc_';
		...
		if (isset($new_company['id'])) $id = (int)sanitize_text_field($new_company['id']);
		if (isset($company_array[$fieldPrefix.'email'])) $email = $company_array[$fieldPrefix.'email'];
		...
		$verifiedID = -1; if ($id > 0){
			...
			$verifiedID = $zbs->DAL->companies->getCompany($id,array(
		...
		if (
			...
			#} ID Exists?
			if ($verifiedID > 0) $update_args['id'] = $verifiedID;

			#} Tags
			if ($we_have_tags) $update_args['tags'] = $company_tags; 

			#} Status default - double-backup for api check
			if (isset($update_args) && is_array($update_args) && (is_null($update_args[$fieldPrefix.'status']) || !isset($update_args[$fieldPrefix.'status']) || empty($update_args[$fieldPrefix.'status']))) {
