   ...
   ====================================================== */
	...
	if (!zeroBSCRM_API_is_zbs_api_authorised()){
		...
		if (is_array($potentialEvent)){
			...
			$eventFields['title'] = ''; if (isset($potentialEvent['title'])) $eventFields['title'] = sanitize_text_field($potentialEvent['title']);
			$eventFields['customer'] = -1; if (isset($potentialEvent['customer'])) $eventFields['customer'] 	= (int)sanitize_text_field($potentialEvent['customer']);

			$eventFields['notes'] = ''; if (isset($potentialEvent['notes'])) $eventFields['notes']		= sanitize_text_field($potentialEvent['notes']);
			$eventFields['to'] = ''; if (isset($potentialEvent['to'])) $eventFields['to'] 			= sanitize_text_field($potentialEvent['to']);
			$eventFields['from'] = ''; if (isset($potentialEvent['from'])) $eventFields['from'] 		= sanitize_text_field($potentialEvent['from']);
			$eventFields['notify'] = -1; if (isset($potentialEvent['notify'])) $eventFields['notify'] 		= (int)sanitize_text_field($potentialEvent['notify']);
			$eventFields['complete'] = -1; if (isset($potentialEvent['complete'])) $eventFields['complete'] 	= (int)sanitize_text_field($potentialEvent['complete']);
			$eventFields['owner'] = -1; if (isset($potentialEvent['owner'])) $eventFields['owner']		= (int)sanitize_text_field($potentialEvent['owner']);

			// this was funky? $eventFields['event_id'] = -1; if (isset($new_event['event_id'])) $eventFields['event_id'] 	= (int)$new_event['event_id'];
			// .. this is a bit cleaner.
			if (isset($potentialEvent['event_id'])) $eventID 	= (int)sanitize_text_field( $potentialEvent['event_id'] );
			if (isset($potentialEvent['id'])) $eventID 	= (int)sanitize_text_field( $potentialEvent['id'] );
		...
		*/
		...
		// DAL3 Notify changes the way it's passed, now gets passed as a reminder
			...
			// get passed (DAL3+)
			$eventReminders = array(); if (isset($new_event['reminders'])) $eventReminders = $new_event['reminders'];
			...
			$eventFields['notify'] = -1; if (isset($new_event['notify'])){
					...
					if ($oldNotify == 24) $eventReminders[] = array(
