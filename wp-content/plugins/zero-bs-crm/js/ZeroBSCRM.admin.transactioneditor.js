		...
		zbscrm_js_getCustInvs(custID, function(r){
			...
			var retHTML = '<select id="invoice_id" name="invoice_id" class="form-control">'; //form-control
				...
				if (r.length > 0){
					...
					retHTML += '<option value="">' + zeroBSCRMJS_transEditLang('none','None') + '</option>';
					...
					jQuery.each(r,function(ind,ele){
						...
						if (zbscrm_JS_DAL() > 2){
							...
							// if ref, that too
							if (typeof ele.id_override != "undefined") invStr += ' - ' + ele.id_override;
					...
					// no invs
					retHTML += '<option value="" disabled="disabled" selected="selected">'+ zeroBSCRMJS_transEditLang('noinvoices','None Found') +'</option>';
		...
		if (inv > 0){
			...
			// seems like a legit inv, add
				...
				var html = '<div class="ui right floated mini animated button zbs-view-invoice" style="margin-left:0.5em">';
						html += '<div class="visible content">' + zeroBSCRMJS_transEditLang('view','View') + '</div>';
	...
	if (typeof contactID != "undefined" && contactID !== null && contactID !== ''){
		...
		if (contactID > 0){
			...
			var url = window.zbsObjectViewLinkPrefixCustomer + contactID;
				...
				var navButton = '<a target="_blank" style="margin-left:6px;" class="zbs-trans-quicknav-contact ui icon button blue mini labeled" href="' + url + '"><i class="user icon"></i> ' + zeroBSCRMJS_transEditLang('contact','Contact') + '</a>';
	...
	if (typeof companyID != "undefined" && companyID !== null && companyID !== ''){
		...
		if (companyID > 0){
			...
			// seems like a legit inv, add
				...
				// ALSO show in header bar, if so
				var navButton = '<a target="_blank" style="margin-left:6px;" class="zbs-trans-quicknav-company ui icon button blue mini labeled" href="' + window.zeroBSCRMJS_transactionedit_links.editcompanyprefix + companyID + '"><i class="building icon"></i> ' + zeroBSCRMJS_transEditLang('company','Company') + '</a>';
				jQuery('#zbs-transaction-learn-nav').append(navButton);
...
function zeroBSCRMJS_transEditLang(key,fallback){
    ...
    if (typeof window.zeroBSCRMJS_transactionedit_lang[key] != "undefined") return window.zeroBSCRMJS_transactionedit_lang[key];
