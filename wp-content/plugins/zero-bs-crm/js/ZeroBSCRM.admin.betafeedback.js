 ...
 */
 jQuery(function(){
    ...
    if (typeof window.zbsBetaFeedback != "undefined") zerobscrmJS_bindBetaFeedback();
    ...
    if (typeof window.zbsBetaFeedback != "undefined"){
    	...
    	if (typeof window.zbsBetaFeedback.title != "undefined"){
    		...
    		var html = '<div class="ui large modal" id="zbs-beta-feedback">';
				...
				html +='    <p id="zbs-beta-feedback-commentwrap">' + window.zbsBetaFeedback.desc + '<br /><textarea id="zbs-beta-feedback-comment" placeholder="' + window.zbsBetaFeedback.commentplaceholder + '"></textarea><br />' + window.zbsBetaFeedback.incdata + '</p>';
				html +='	<p style="display:none" id="zbs-beta-feedback-sent">' + window.zbsBetaFeedback.sent + '</p>';
				html +='	<p style="display:none" id="zbs-beta-feedback-fail">' + window.zbsBetaFeedback.fail + '</p>';
				html +='	<div class="ui dimmer" id="zbs-beta-feedback-loader"><div class="ui loader"></div></div>';
				html +='  </div>';
				html +='  <div class="actions">';
				html +='  	<div class="ui labeled input left floated">';
				html +='	  <div class="ui label">' + window.zbsBetaFeedback.emaillabel + '</div>';
				html +='	  <input type="text" id="zbs-beta-feedback-email" value="' + window.zbsBetaFeedback.email + '" placeholder="' + window.zbsBetaFeedback.emailplaceholder + '">';
				html +='	</div>';
				html +='    <div class="ui black button" id="zbs-cancel-beta-feedback">' + window.zbsBetaFeedback.cancelbutton + '</div>';
				html +='    <div class="ui green right labeled icon button" id="zbs-send-beta-feedback">' + window.zbsBetaFeedback.sendbutton + '<i class="checkmark icon"></i></div>';
				html +='  </div>';
				html +='</div>';

			// attention grabbing ico!
			html += '<button class="ui black button" id="zbs-beta-feedback-request">' + window.zbsBetaFeedback.logo + window.zbsBetaFeedback.givefeedback + '</button>';
			...
			setTimeout(function(){
				...
				jQuery('#zbs-send-beta-feedback').on( 'click', function(){
					...
					// go.
					zerobscrmJS_sendBetaFeedback(function(r){
 ...
 function zerobscrmJS_sendBetaFeedback(successcb,errcb){
    ...
    if (!window.zbsBetaFeedbackInTube){
        ...
        window.zbsBetaFeedbackInTube = true;
        	...
        	// page if present
        	var page = ''; if (typeof window.zbsPageKey != "undefined") page = window.zbsPageKey;
        	var area = ''; if (typeof window.zbsBetaFeedback.area != "undefined") area = window.zbsBetaFeedback.area; 
