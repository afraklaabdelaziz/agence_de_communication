...
function zerobscrmJS_bindMetaboxManager(){
    ...
    // minimise/show
    jQuery('.zbs-metabox-minimise').off('click').on( 'click', function(){
...
function zeroBSCRMJS_buildScreenOptionsMetaboxes(){
    ...
    // empty defaults
    var newScreenOptions = { mb_normal: {}, mb_side: {}, mb_hidden: [], mb_mini:[], pageoptions:[] }; 
            ...
            jQuery.each(mbAreas,function(mbAreaIndx,mbArea){
                ...
                var obj = {};
                  ...
                  jQuery('#zbs-' + mbArea + '-sortables .zbs-metabox-tabgroup').each(function(ind,ele){
                        ...
                        if (typeof tabgroupID == "undefined" || tabgroupID == '') tabgroupID = 'tabs_' + tabIdx;
...
function saveMetaBoxes(){
  ...
  // TABS!
    ...
    /* This tabs them up */
      ...
      var obj = {}; obj['tabs_1'] = tabbedList.join(',');
      newScreenOptions.mb_normal.push(obj)


    
    // side section
    if (typeof newScreenOptions.mb_side == "undefined") newScreenOptions.mb_side = [];
    ...
    // hidden metaboxes
    if (typeof newScreenOptions.mb_hidden == "undefined") newScreenOptions.mb_hidden = [];
    jQuery('.zbs-metabox').each(function(ind,ele){

      // if has class 'zbs-hidden' add to list
      if (jQuery(ele).hasClass('zbs-hidden')) newScreenOptions.mb_hidden.push(jQuery(ele).attr('id'))

    });
    
    // minimised metaboxes
    if (typeof newScreenOptions.mb_mini == "undefined") newScreenOptions.mb_mini = [];
    jQuery('.zbs-metabox').each(function(ind,ele){

      // if has class 'zbs-minimised' add to list
      if (jQuery(ele).hasClass('zbs-minimised')) newScreenOptions.mb_mini.push(jQuery(ele).attr('id'))
