...
function zbscrm_JS_retrieve_invoice_data(id){
	...
	jQuery.ajax({
		...
		success: function (res) {
			...
			// localise inv data
			if (typeof res != "undefined") window.zbs_invoice = res;

			// localise tax table data
			if (typeof res.tax_linesObj != "undefined") window.zbs_tax_table = res.tax_linesObj;

			// localise tax data
			if (typeof res.invoiceObj != "undefined" &&
				typeof res.invoiceObj.settings != "undefined" &&
				typeof res.invoiceObj.settings.invtax != "undefined") window.zbs_tax = res.invoiceObj.settings.invtax;
...
function zbscrm_JS_draw_invoice_html(){
	...
	if (typeof window.zbs_invoice != "undefined" && window.zbs_invoice !== false){
		...
		setTimeout(function(){
			...
			jQuery('#zbs-invoice-custom-fields-holder table tr').appendTo('.zbs-invoice-topper table');
...
function zbscrm_JS_draw_invoice_actions_html(res){
	...
	html += '<div id="zbs_invoice_actions">';
		...
		if (!res.invoiceObj.new_invoice){
			...
			if (res.invoiceObj.portal_installed) html += '<a href="' + res.invoiceObj.preview_link + '" target="_blank" class="ui button blue" id="zbs_invoice_preview">' + zbscrm_JS_invoice_lang('preview')  +'</a>';

			//pdf download only displayed in PDF set.
			if (res.invoiceObj.pdf_installed){
				html += '<button id="zbs_invoicing_download_pdf" type="button" class="ui button olive">' + zbscrm_JS_invoice_lang('dl_pdf') +'</button>';
				Formhtml = '<form target="_blank" method="post" id="zbs_invoicing_download_pdf_form" action="">';
			...
			if (typeof window.zbsJS_invEmailActive != "undefined" && window.zbsJS_invEmailActive == 1){
				...
				if (typeof potentialEmail != "undefined" && potentialEmail != '' && zbscrm_JS_validateEmail(potentialEmail)){
					
					html += '<button type="button" id="zbs_invoicing_send_email" class="ui button yellow">' + zbscrm_JS_invoice_lang('send_email') +'</button>';			
...
function zbscrmJS_retrieveCurrentBillToEmail(){
	...
	if ( typeof potentialEmail == "undefined" || ! zbscrm_JS_validateEmail( potentialEmail ) ) {
		...
		if ((billTo == null || billTo == '') && typeof window.zbsJS_prefillemail != "undefined" && typeof window.zbsJS_prefillid != "undefined"){
...
function zbscrm_JS_draw_invoice_logo_html(res){
	...
	html = '<div id="zbs_invoice_logos">';
		html += '<div class="wh-logo '+hide+'">';
			html += '<i class="fa fa-file-image-o" aria-hidden="true"></i><span class="wh-logo-text">+ ' + zbscrm_JS_invoice_lang('add_logo') + '</span>';
		html += '</div>';
		html += '<div class="wh-logo-set '+show+'">';
			html += '<img id="wh-logo-set-img" src="' + res.invoiceObj.logo_url + '" />';
			html += '<input type="hidden" name="zbsi_logo" id="logo" value="' + res.invoiceObj.logo_url + '" />';
	...
	html += '<div class="zbs-invoice-topper">';
		html += '<table class="form-table">';
			...
			if (res.invoiceObj.settings.hideid != "1"){
            	...
            	var potentialInvID = ''; var zbsID = -1;
            	if (typeof res.invoiceObj.id != "undefined") { potentialInvID = res.invoiceObj.id; zbsID = res.invoiceObj.id; }
            	...
            	if (potentialInvID == -1){
            		...
            		// show
					html += '<tr class="wh-large zbs-invoice-number"><th><label for="no">'+ zbscrm_JS_invoice_lang('invoice_number') +':</label></th>';
						html += '<td>';
			...
			html += '<tr class="wh-large"><th><label for="date">'+zbscrm_JS_invoice_lang('invoice_date') + ':</label></th>';
				html += '<td>';
					html += '<input type="text" name="zbsi_date" id="date" class="form-control zbs-date zbs-empty-start" placeholder="" value="' + res.invoiceObj.date_date + '" data-original-date="' + res.invoiceObj.date_date +'" data-utc="' + res.invoiceObj.date +'" />';
                html += '</td>';
            html += '</tr>';
			...
			html += '<th><label for="ref">' + zbscrm_JS_invoice_lang('reference') + ':</label></th>';
				...
				if ( 'reftype' in res.invoiceObj.settings && res.invoiceObj.settings.reftype === 'autonumber' ) {
					if ( res.invoiceObj.status === 'draft' ) {
						...
						html += '<span style="color:grey" title="' + zbscrm_JS_invoice_lang('nextref') + '">' + zbscrm_JS_invoice_lang('autogenerated') + ': ' + prefix + next_number + suffix +'</span>';
						if ( is_first_invoice && prefix === '' && suffix === '' ) {
							html += '<br><a href="' + res.invoiceObj.settings.settings_slug + '"> ' + zbscrm_JS_invoice_lang('refsettings' ) + '</a>';
						}
					...
					html += '<input type="text" name="zbsi_ref" id="ref" class="form-control widetext" placeholder="" value="' + res.invoiceObj.id_override + '" autocomplete="zbsinv" />';
				}
				html += '</td>';
            html += '</tr>';
            ...
            if (typeof res.invoiceObj.due_date != "undefined" && res.invoiceObj.due_date != null && res.invoiceObj.due_date != 0 ){

            	// due on date <datepicker>
				html += '<tr class="wh-large"><th><label for="due_date">'+zbscrm_JS_invoice_lang('due_date') + ':</label></th>';
					html += '<td>';
						html += '<input type="text" name="zbsi_due_date" id="due_date" class="form-control zbs-date" placeholder="" value="' + res.invoiceObj.due_date_date + '" data-original-date="' + res.invoiceObj.due_date_date +'" />';
	                html += '</td>';
	            html += '</tr>';
				...
				html += '<th><label for="due">' + zbscrm_JS_invoice_lang('due_date') +  ':</label></th>';
					html += '<td>';
						html += '<select id="zbsinv_due_days" name="zbsi_due" class="form-control" style="font-size:16px;">';
							...
							html += '<option value = "45">' + zbscrm_JS_invoice_lang('due','','fortyfive') + '</option>';
			...
			// custom fields
			if (typeof window.zbscrmjs_invoice_custom_fields != "undefined" && window.zbscrmjs_invoice_custom_fields.length > 0){
...
function zbscrm_JS_draw_send_invoice_to(res){
	...
	// ... if so, interpret
	if (typeof zbs_invoice.invoiceObj.invoice_contact == "object" && typeof zbs_invoice.invoiceObj.invoice_contact.id !== "undefined") zbs_invoice_contact = parseInt(zbs_invoice.invoiceObj.invoice_contact.id);
	if (typeof zbs_invoice.invoiceObj.invoice_company == "object" && typeof zbs_invoice.invoiceObj.invoice_company.id !== "undefined") zbs_invoice_company = parseInt(zbs_invoice.invoiceObj.invoice_company.id);
	...
	if ((billTo == null || billTo == '') && typeof window.zbsJS_prefillemail != "undefined" && typeof window.zbsJS_prefillid != "undefined"){
		...
		if (typeof window.zbsJS_prefillobjtype != "undefined"){
			...
			if (window.zbsJS_prefillobjtype == 1 && zbs_invoice_contact == -1) zbs_invoice_contact = parseInt(window.zbsJS_prefillid);
			if (window.zbsJS_prefillobjtype == 2 && zbs_invoice_company == -1) zbs_invoice_company = parseInt(window.zbsJS_prefillid);

			// catch any mishaps
			if (zbs_invoice_contact <= 0) zbs_invoice_contact = -1;
			if (zbs_invoice_company <= 0) zbs_invoice_company = -1;
		}
	...
	var html = '';
		...
		html += '<div class="billing-to-title">';
			html += '<i class="linkify icon"></i> <span class="your-info-biz">'+ zbscrm_JS_invoice_lang('send_to') +'</span>';
		html += '</div>';
			...
			//html += '<div class="inv-to-form">' + zbscrm_JS_invoice_lang('send_to') +':</div>';
                html += '<div class="wh-large inv-to-input">';
						...
						html += '<input class="form-control typeahead" type="text" id="zbs_inv_bill" name="zbscq_bill" value="' + billTo + '" placeholder="' + zbscrm_JS_invoice_lang('bill_to') + '" />';
						html += '</div>';
						html += '<div class="edit-c"><a class="zbs-edit-assign-to" href="#">' + zbscrm_JS_invoice_lang('edit_record') + '</a></div>';
                html += '</div>';
            html += '</div>';
		html += '</div>';
		html += '<div class="clear"></div>';
		...
		html += '<input type="hidden" id="zbs_invoice_contact" name="zbs_invoice_contact" value="'+ zbs_invoice_contact +'" />';
		html += '<input type="hidden" id="zbs_invoice_company" name="zbs_invoice_company" value="' + zbs_invoice_company + '" />';
	...
	html += '<div id="zbs-invoice-customiser">';
		html += '<div class="ui grid" style="margin: 0em 0.5em;">';
			html += '<div class="eight wide column" style="padding:0">';
				html += '<span class="header" style="float:left;">' + zbscrm_JS_invoice_lang('customise')  +'</span>';
				html += '<select class="form-control" id="invoice-customiser-type" name="invoice-customiser-type" id="invoice-customiser-type" style="width:30%;">';
					html += '<option value="quantity"'; 
					if (res.invoiceObj.invoice_hours_or_quantity == 'quantity') html += ' selected="selected"';
					html += '>' + zbscrm_JS_invoice_lang('quantity') + '</option>';
					html += '<option value="hours"'; 
					if (res.invoiceObj.invoice_hours_or_quantity == 'hours') html += ' selected="selected"';
...
function zbscrm_JS_draw_invoice_biz_info(res){
	...
	html += '<div id="zbs-business-info-wrapper">';
		html += '<div class="business-info-toggle">';
			html += '<i class="fa fa-chevron-circle-right" aria-hidden="true"></i> <span class="your-info-biz">'+ zbscrm_JS_invoice_lang('biz_info') +'</span>';
		html += '</div>';
		...
		html += '<div class="business-info">';
			html += '<table class="table zbs-table">';
				...
				html += '</tbody>';
			html += '</table>';
			html += '<span class="edit-or-add"><a href="' + res.invoiceObj.settings.biz_settings_slug  +'" target="_blank">' + zbscrm_JS_invoice_lang('add_edit') + '</a></span>';
		html += '</div>';
	html += '</div>';
...
function zbscrm_JS_draw_line_items(res){
	...
	html += '<div id="zbs-invoice-items">';
		...
		//the add new row UI.
		html += '<div id="zbs-add-new-row"><i class="plus circle icon"></i>'+ zbscrm_JS_invoice_lang('add_row')  +'</div>';
	...
	html += '<div class="invoice-grand-total-wrapper ui grid">';
		...
		html += '<div class="col-md-6 inv-totals eight wide column">';
			...
			html += '<div class="total-row">';
				html += '<div class="zlabel half">' + zbscrm_JS_invoice_lang('sub_total') + "</div>";
			...
			if (res.invoiceObj.settings.invdis == 1) {
				html += '<div class="total-row">';
					...
					html += '<input class = "form-control half" type="number" name="invoice_discount_total" id="invoice_discount_total" step="0.01" min="0" value="' + res.invoiceObj.totals.invoice_discount_total  +'">';
					if (res.invoiceObj.totals.invoice_discount_type == 'm'){ 
						html += '<select id="invoice_discount_type" name="invoice_discount_type" class="form-control half"><option value="%" >%</option><option value="m" selected>'+ zbscrm_JS_invoice_lang('amount') +'</option></select>';
					} else {
						html += '<select id="invoice_discount_type" name="invoice_discount_type" class="form-control half"><option value="%">%</option><option value="m">' + zbscrm_JS_invoice_lang('amount') + '</option></select>'				
					}
					html += '</div>';
			...
			if (res.invoiceObj.settings.invpandp == 1) {
				html += '<div class="total-row pbot30">';
					...
					html += '<input class="zbs_gt zbsli_price-1 half ri topmm5" type="number" step="0.01" min="0" name="invoice_postage_total" id="invoice_postage_total" value="' + res.invoiceObj.totals.invoice_postage_total +'">';
				html += '</div>';
			}
			...
			if (res.invoiceObj.settings.invpandp == 1 && res.invoiceObj.settings.invtax == 1) {
				...
				html += '<div class="total-row pbot30">';
					...
					html += '<div class="half ri topmm10">' + zbscrm_JS_output_tax_line(res, -1, shipping) + '</div>';
				html += '</div>';
			}
			...
			html += '<div class="total-row grand-total">';
				...
				html += '<input type="hidden" name="zbs-inv-grand-total-store" id="zbs-inv-grand-total-store" value="0" />';
			html += '</div>';
...
function zbscrm_JS_draw_partials_table(res){
	...
	var html = '<div id="zbs-invoice-partial-payments" class = "ui grid">';
		...
		jQuery.each(res.invoiceObj.partials, function (i, v) {
			...
			if (i == 0){
				//first row
				html += '<div class="partial-row-tab">' + zbscrm_JS_invoice_lang('partial_table') + '</div>';
			}
			html += '<div class="partial-row">';

			var vTypeStr = ''; // if != 'sale' this'll add type
			var vFailStr = ''; // if it's not 'succeeded' status will add
			if (typeof v.type != "undefined" && typeof v.type_accounting != "undefined" && v.type_accounting == 'credit') vTypeStr = ' (' + v.type + ')';
			var vSymbolStart = '', vSymbolEnd = ''; // this gets set to () if is a creditnote/refund transaction
			...
			// failed trans ignore...
			var vStatus = zbscrm_JS_invoice_lang('incomplete','Incomplete'); if (typeof v.status != "undefined") vStatus = v.status;
			...
			html += '<div class="zlabel half"><a href="'+ link  +'" target="_blank">' + v.ref + vTypeStr + '</a>' +  vFailStr + '</div>';
			html += '<div class="zbs-partial-value half ri">' + parseFloat(partial_total_value).toFixed(zbs_root.currencyOptions.noOfDecimals) + '</div>';
			html += '</div>';
		...
		if (window.invoice_partial){
			html += '<div id="amount-due" class="due-row">';
				html += '<div class="amt-due-label zlabel half">' + zbscrm_JS_invoice_lang('amount_due') + '</div>';
...
function zbscrm_JS_generate_invoice_row(res, i, v){
	...
	var rowhtml = '<tr class="zbs-invoice-row zbs-item-block zbs-rowid' + i + '" data-rowid="' + i + '">';
	//title test
	if (v.title == null) {
		item_title = '';
	} else {
	...
	rowhtml += '<input class="form-control zbs-item-name" type="text" name = "zbsli_itemname[]" value = "' + item_title + '" placeholder="' + zbscrm_JS_invoice_lang('rowtitleplaceholder') + '" /><br/>';
	rowhtml += '<textarea class="form-control" name = "zbsli_itemdes[]" placeholder="' + zbscrm_JS_invoice_lang('rowdescplaceholder') + '">' + item_description + '</textarea>';
	rowhtml += '</td>';

	//quantity column
	rowhtml += '<td class="second">';
	rowhtml += '<input type="number" class="zbsli_quan zbsli_quan'+i+' form-control quan" data-zbsr="' + i + '" name = "zbsli_quan[]" value = "' + v.quantity + '" min="0">';
	rowhtml += '</td>';

	//price column
	rowhtml += '<td class="third">';
	rowhtml += '<input type="number" class="zbsli_price zbsli_price' + i +' form-controm price" data-zbsr="' + i + '" name = "zbsli_price[]" value = "' + v.price + '" step="0.01">';
	rowhtml += '</td>';
	...
	rowhtml += '<td><div class="remove-row" data-rowid="' + i + '"><i class="times circle icon"></i></div></td>';
...
function zbscrm_JS_output_tax_line(res, i, v){
	...
	if (i == -1){
		taxhtml = '<select name="zbsli_tax_ship" class="tax-select tax-select'+ i +' form-control" data-zbsr="'+ i +'">';
	} else {
		taxhtml = '<select name="zbsli_tax[]" class="tax-select tax-select' + i + ' form-control" data-zbsr="' + i + '">';
	}
	...
	if (tax_id == 0){
		...
		taxhtml += '<option value="0" ' + selected + '>' + zbscrm_JS_invoice_lang('no_tax') + '</option><optgroup label="' + zbscrm_JS_invoice_lang('taxgrouplabel') + '">'; 
			jQuery.each(res.tax_linesObj, function (j, t) {
				...
				taxhtml += '<option value="'+  t.id  +'" '+ selected +'>' + t.name + ' : ' + t.rate + '%</option>'; 
...
function zbscrm_JS_bind_due_days(){
	...
	if (typeof window.zbs_invoice !== "undefined" && 
		typeof window.zbs_invoice.invoiceObj !== "undefined" &&
		typeof window.zbs_invoice.invoiceObj.due !== "undefined") jQuery('#zbsinv_due_days').val(window.zbs_invoice.invoiceObj.due);
...
function zbscrm_JS_bind_row_actions(){
	...
	// add empty row
	jQuery('#zbs-add-new-row').off('click').on('click', function(){
	...
	// remove a row.
	jQuery('.remove-row').off('click').on('click',function(e){
	...
	jQuery(".zbs-item-block input[type=number]").on('keyup mouseup', function () {
		...
		row_tot = quan * price;
		jQuery(this).parent().siblings('.row-amount-' + zbs_row_to_up).html(row_tot.toFixed(zbs_root.currencyOptions.noOfDecimals));
...
function zbscrm_JS_calculate_invoice_row_subtotals(){
	...
	jQuery('.zbs-invoice-row').each(function(index, pvalue){
		...
		row_tot = quan * price;
		jQuery('.row-amount-' + zbs_row_to_up).html(row_tot.toFixed(zbs_root.currencyOptions.noOfDecimals));
...
function zbscrm_JS_calculate_invoice_subtotal(){
	...
	jQuery('#subtotal-value').html(invoice_subtotal.toFixed(zbs_root.currencyOptions.noOfDecimals));
...
function zbscrm_JS_calculate_invoice_tax_table(){
	...
	var total_amount = 0, this_tax_id = -1, this_row_index = -1;
...
function zbscrm_JS_calculate_tax_amounts(this_row_amount, tax_percent, this_shipping_tax){
	...
	if (total > 0){
		...
		for (var i = 0; i < this_row_amount.length; i++) {
			...
			//console.log('shipping',this_shipping_tax[i]);
			if (this_shipping_tax[i] == 1){
				discount_proportion = 0;
			} else {
				discount_proportion = discount_amount / total;
			}
			if (this_row_amount[i] > 0){
				if (tax_percent[i] == 0){
	...
	jQuery.each(tax_table_output, function (i, v) {
		if (i > 0){
			...
			tax_html += '<div class="total-row row">';
				...
				tax_html += '<div class="tax-amt zbs-total-tax third ri">' + tax_table_output[i].toFixed(zbs_root.currencyOptions.noOfDecimals) + '</div>';	
			tax_html += '</div>';
		}
	});
...
function zbscrm_JS_calc_grandtotal(){
		...
		if (debugTotalling) console.log('=========== Totals: ===========');
		if (debugTotalling) console.log('Subtotal:' + zbs_gt_invoice_tot);
		...
		if (window.zbs_invoice.invoiceObj.settings.invdis == 1) {
			...
			if (debugTotalling) console.log('Discounted ' + parseFloat(jQuery('#discount-value').html()) + ': ' + zbs_gt_invoice_tot);
		}

		//step 3: add shipping
		if (window.zbs_invoice.invoiceObj.settings.invpandp == 1) {
			zbs_gt_invoice_tot = zbs_gt_invoice_tot + parseFloat(jQuery('#invoice_postage_total').val());
			if (debugTotalling) console.log('+Shipping ' + parseFloat(jQuery('#invoice_postage_total').val()) + ': ' + zbs_gt_invoice_tot);
		}
		...
		if (window.zbs_invoice.invoiceObj.settings.invtax == 1) {
			jQuery('.zbs-total-tax').each(function(i,v){
				...
				if (!isNaN(taxValue)) zbs_gt_invoice_tot = zbs_gt_invoice_tot + taxValue;

				if (debugTotalling) console.log('+TAX ' + parseFloat(jQuery(this).html()) + ': ' + zbs_gt_invoice_tot);
			});
		}

		if (debugTotalling) console.log('TOTAL:' + zbs_gt_invoice_tot.toFixed(zbs_root.currencyOptions.noOfDecimals));
		if (debugTotalling) console.log('=========== / Totals ==========');

		jQuery('#zbs-inv-grand-total').html(zbs_gt_invoice_tot.toFixed(zbs_root.currencyOptions.noOfDecimals));
		...
		// SHOULD BE REPLACED with php variant of js total calc code here, later
		// ... smt like zeroBSCRM_invoicing_calcTotal($inv);
		jQuery('#zbs-inv-grand-total-store').val(zbs_gt_invoice_tot.toFixed(zbs_root.currencyOptions.noOfDecimals));
...
function zbscrm_JS_bind_change_actions(){
	...
	jQuery('#invoice_postage_total').on( 'change', function(){
		...
		jQuery('#pandptotal').html(window.zbs_root.currencyOptions.currencyStr+parseFloat(jQuery(this).val()).toFixed(zbs_root.currencyOptions.noOfDecimals));
		// recalc
		zbscrm_JS_calcTotals();
...
function zbscrm_JS_calculatediscount(){
	...
	if (zbs_discount_amt != ''){
		if (zbs_discount_type == 'm'){
			jQuery('#invoice_discount_total_value').val(parseFloat(zbs_discount_amt).toFixed(zbs_root.currencyOptions.noOfDecimals));
			jQuery('#discount-value').html(parseFloat(zbs_discount_amt).toFixed(zbs_root.currencyOptions.noOfDecimals));
		} else {
			var zbs_discount_combi = parseFloat(jQuery('#subtotal-value').html()) * zbs_discount_amt/100;
			jQuery('#invoice_discount_total_value').val(parseFloat(zbs_discount_combi).toFixed(zbs_root.currencyOptions.noOfDecimals));
			jQuery('#discount-value').html(parseFloat(zbs_discount_combi).toFixed(zbs_root.currencyOptions.noOfDecimals));
		}
	}
...
function zbscrm_JS_invoice_typeahead_bind(){
	...
	jQuery('#billing-to .typeahead').typeahead({
		minLength: 1,
		highlight: true,
		hint: true,
		},{
		...
		display: function (r) {
			if (r.name.trim()) return r.name; //company name is required, so will always return this
			else if (r.email) return r.email;
			return 'Contact #' + r.id;
		},
		...
		templates: {
			suggestion: function (r) {
				...
				var name = r.name.trim() ? r.name : zeroBSCRMJS_globViewLang('contact') + ' #' + r.id;
				var email = r.email ? r.email : '<i>'+zbscrm_JS_invoice_lang('noemail')+'</i>';
				sug = '<div class="sug-wrap"><div class="ico">' + ico + '</div><div class="inner"><div class="name">' + name + '</div><div class="email">' + email + '</div></div><div class="clear"</div></div>';
				return sug;
			},
			empty: function (v){
				//to do - link this to the add new customer (if B2B mode should also show link to add new Company too? hmmm WH thoughts? (needs translating too)
					var str = '<a href="' + window.zbs_invoice.invoiceObj.settings.addnewcontacturl + '" target="_blank">' + zbscrm_JS_invoice_lang('addnewcontact') + '</a>';
					if (window.zbs_invoice.invoiceObj.settings.b2bmode){
					 str += ' ' + zbscrm_JS_invoice_lang('or') + ' ';
					 str += '<a href="' + window.zbs_invoice.invoiceObj.settings.addnewcompanyurl + '" target="_blank">' + zbscrm_JS_invoice_lang('newcompany') + '</a>';
					}
				return '<div class="zbs-add-new-js-assign-to"><i class="address book icon"></i>  ' + str + '</div>';
			}
		}
	}).on('typeahead:selected', select).on('typeahead:change', change);
	...
	//autocomplete="zbs-concom-time' + window.zbscrm_js_autocomplete_chaff + '"
		...
		setTimeout(function(){
			...
			var k = jQuery('#billing-to .typeahead').attr('data-autokey'); if (typeof k == "undefined") var k = '-typeahead';
			var ns = 'zbsobj-' + utc + '-' + k;
...
function zbscrm_JS_bindInitialLearnLinks(){
	...
	// show hide learn menu links
	if (typeof window.zbs_invoice.invoiceObj != "undefined" &&
		...
		window.zbs_invoice.invoiceObj.invoice_contact != "-1"){
			...
			if (cID > 0) zeroBSCRMJS_showContactLinkIf(cID);
	}
	if (typeof window.zbs_invoice.invoiceObj != "undefined" &&
		...
		window.zbs_invoice.invoiceObj.invoice_company != "-1"){
			...
			if (coID > 0) zeroBSCRMJS_showCompanyLinkIf(coID);
	}
		...
		if (contactID > 0){
			...
			// seems legit, add
				...
				*/
				...
				var navButton = '<a target="_blank" style="margin-left:6px;" class="zbs-invoice-quicknav-contact ui icon button blue mini labeled" href="' + window.zbs_invoice.invoiceObj.settings.contacturlprefix + contactID + '"><i class="user icon"></i> ' + zbscrm_JS_invoice_lang('contact') + '</a>';
		...
		if (companyID > 0){
			...
			// seems like a legit inv, add
				...
				*/
				...
				// ALSO show in header bar, if so
				var navButton = '<a target="_blank" style="margin-left:6px;" class="zbs-invoice-quicknav-company ui icon button blue mini labeled" href="' + window.zbs_invoice.invoiceObj.settings.companyurlprefix + companyID + '"><i class="building icon"></i> ' + zbscrm_JS_invoice_lang('company') + '</a>';
				jQuery('#zbs-invoice-learn-nav').prepend(navButton);
...
function zbscrm_JS_bind_invoice_actions(){
	...
	// Show Hides
	jQuery('.zbs-add-memo-trigger').off('click').on('click', function(e){
		jQuery('.zbs-memo-box').show();
		jQuery('.zbs-add-memo-trigger').hide();
	});
	jQuery('.zbs-memo-hide').off('click').on('click',function(e){
		jQuery('.zbs-memo-box').hide();
		jQuery('.zbs-add-memo-trigger').show();

	});
	jQuery('.wh-logo-set .zbs-remove').off('click').on('click',function(e){
	...
	jQuery('.business-info-toggle').off('click').on('click',function(e){
		e.preventDefault();
	...
	// Logo 
	jQuery('.wh-logo, .wh-logo-set .zbs-update').off('click').on('click',function(e){
	...
	// Download
	jQuery('#zbs_invoicing_download_pdf').off("click").on("click", function (e) {
		e.preventDefault();
	...
	// send email button
	jQuery('#zbs_invoicing_send_email').off('click').on( 'click', function(ind,ele){
	...
	if (typeof invEmail != "undefined" && invEmail != '' && zbscrm_JS_validateEmail(invEmail)){
		...
		var optsHTML = '<div id="zbs_invoice_email_modal_opts">';
			...
			optsHTML += '<div class="zbs-invoice-email-modal-field">';
				optsHTML += '<label for="zbs_invoice_email_modal_toemail">' + zbscrm_JS_invoice_lang('toemail') + '</label>';
				optsHTML += '<input type="email" id="zbs_invoice_email_modal_toemail" value="' + invEmail + '" placeholder="' + zbscrm_JS_invoice_lang('toemailplaceholder') + '" />';
			optsHTML += '</div>';
			...
			if (jQuery('.zbsFileLine').length > 0){
				...
				var checkedStr = ''; if (jQuery('#zbsc_sendattachments').is(':checked')) checkedStr = 'checked="checked" ';
					optsHTML += '<input type="checkbox" id="zbs_invoice_email_modal_attachassoc" value="1" ' + checkedStr + '/>';
					optsHTML += '<label for="zbs_invoice_email_modal_attachassoc">' + zbscrm_JS_invoice_lang('attachassoc') + '</label>';
				optsHTML += '</div>';
			}
			...
			optsHTML += '<div class="zbs-invoice-email-modal-field">';
				optsHTML += '<input type="checkbox" id="zbs_invoice_email_modal_attachaspdf" value="1" ' + checkedStr + '/>';
				optsHTML += '<label for="zbs_invoice_email_modal_attachaspdf">' + zbscrm_JS_invoice_lang('attachpdf') + '</label>';
			optsHTML += '</div>';
		...
		swal({
			...
			html: '<div class="ui segment">' + zbscrm_JS_invoice_lang('sendthisemail') + optsHTML + '</div>',
			type: 'question',
			...
			if (result.value){
				...
				if (typeof invEmail != "undefined" && invEmail != '' && zbscrm_JS_validateEmail(invEmail) && window.invoice_id > 0){

					// get settings
					var attachassoc = -1;
					if (jQuery('#zbs_invoice_email_modal_attachassoc').length > 0 && jQuery('#zbs_invoice_email_modal_attachassoc').is(':checked')) {
						attachassoc = 1;
					}
					var attachpdf = -1;
					if (jQuery('#zbs_invoice_email_modal_attachaspdf').length > 0 && jQuery('#zbs_invoice_email_modal_attachaspdf').is(':checked')) {
						attachpdf = 1;
					}
					var params = {
		...
		if (typeof params.id != "undefined" && params.id > 0 && typeof params.email != "undefined" && zbscrm_JS_validateEmail(params.email)){
            ...
            jQuery.ajax({
					...
					success: function(response) { 
						...
						// done			
						swal(
						  zbscrm_JS_invoice_lang('senttitle'),
						  zbscrm_JS_invoice_lang('sent'),
						  'info'
						);
...
function zeroBSCRMJS_invEditLang(key){
	if (typeof window.zbsInvoiceBuilderLang[key] != "undefined") return window.zbsInvoiceBuilderLang[key];
	return '';
}
...
function zbscrm_JS_invoice_lang(key,fallback,subkey){
    ...
    if (typeof window.zbs_invoice.invoiceObj.settings.lang[key] != "undefined") {
    	...
    	else if (typeof window.zbs_invoice.invoiceObj.settings.lang[key][subkey] != "undefined") 
