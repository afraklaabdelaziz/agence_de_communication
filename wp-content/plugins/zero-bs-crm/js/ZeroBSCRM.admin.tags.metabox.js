...
function zeroBSCRMJS_buildTags(){
    ...
    if (typeof window.zbsCRMJS_currentTags == "object") jQuery.each(window.zbsCRMJS_currentTags,function(ind,ele){
		...
		if (typeof window.zbsDontDrawTags == "undefined") zbsJS_drawTag(ele,-1);        
...
function zbsJS_drawTag(tagStr,tagID){
    ...
    var html = '<div class="ui small basic label teal" data-id="' + tagID + '"><i class="window close icon zbs-remove-tag"></i> <span>' + tagStr + '</span></div>';
...
function zbsJS_bindTags(){
    ...
    // remove tag
    jQuery('.zbs-remove-tag').off('click').on( 'click', function(){
...
function zbsJS_bindTagsInit(){
    ...
    // add tags
    jQuery('#zbs-add-tag-action').off('click').on( 'click', function(){
    ...
    jQuery('#zbs-tag-suggestions-show-more').off('click').on( 'click', function(){
    ...
    jQuery('#zbs-tag-suggestions-show-less').off('click').on( 'click', function(){
    ...
    // use a suggestion
    jQuery('.zbsTagSuggestion').off('click').on( 'click', function(){
...
function zbsJS_bindTagManagerInit(){
    ...
    jQuery('#zbs-add-tag-action').off('click').on( 'click', function(){
        ...
        if (newTag != '' && jQuery.inArray(newTag,window.zbsCRMJS_currentTags) == -1){
            ...
            // not already loaded, add
            zbsJS_addEmptyTag(newTag,function(r){
                ...
                // add to table
                var tagTR = '<tr><td><span class="ui large blue label">' + ltag + '</span></td><td>' + newTagSlug + '</td><td class="center aligned">0</td><td class="center aligned"><button type="button" class="ui mini button orange zbs-delete-tag" data-tagid="' + newTagID + '"><i class="trash alternate icon"></i> ' + window.zbsTagListLang.delete + '</button></td></tr>';
...
function zeroBSCRMJS_tagManager_bindTagEditButtons(){
  ...
  jQuery('.zbs-delete-tag').off( 'click' ).on( 'click', function(){
