                    ...
                    jQuery.ajax({
                      ...
                      success: function(response) {
                          ...
                          swal(
                            ...
                            'Your email thread has been removed.',
                            'success'
                          );
    ...
    jQuery.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.tasks,function(k,v){
                        total_tasks++;
                        if(v.complete == 1){
                            $('.the-tasks').append('<li class="complete"><i class="ui icon check green circle"></i>'+ v.title + '</li>');
                            completed_tasks++;
                        }else{
                ...
                //the email stuff
                $('.zbs-date').html(response.email_date);
             ...
             //   $('.zbs-email-body-content').append(response.email);
                $.each(response.email, function(k, v) {
                    $('.zbs-email-thread').append('<div class="zbs-date>' + v.date + "</div>");
                    $('.zbs-email-thread').append('<div class="zbs-email-subject em-sub-'+k+'">' + v.zbsmail_subject + "</div>");
                    $('.zbs-email-thread').append('<div class="zbs-email-body-content '+ v.in_or_out + '" id="zbsbody'+ v.the_id +'">' + unescape(v.zbsmail_content) + "</div>");
                    $('#zbsbody' + v.the_id).append('<div class="pointer'+v.in_or_out+'"></div>');
                    $('.zbs-email-thread').append('<div class="email-avatar avatar-'+v.in_or_out+'">'+v.avatar+'</div>');
                    $('.zbs-email-thread').append('<div class="clear"></div>');

                    if(v.zbsmail_opened > 0){
                        $('.zbs-email-thread').append('<div class="zbsread'+v.in_or_out+'"><span class="bold">Read:</span> '+v.zbsmail_lastopened+'</div>');
                    }

                    /*
                                    <div class='zbs-date'></div>
            ...
            error: function(response){
                ...
                $('.spinner-gif').hide();
                jQuery('#zbs-couldnotload').remove();
                jQuery('.zbs-ajax-loading').append('<p id="zbs-couldnotload">' + zbsEmailSingleLang.couldnotload + '</p>');
            }
        ...
        var data = {
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                        ...
                        $('.zbs-ajax-loading').show();
                        $('.zbs-ajax-loading').append('<div class="reply-sent"><i class="ui icon check green circle"></i></div>');
...
function zbscrmjs_customer_setCustomerEmail(obj){
    if (typeof obj.id != "undefined"){
        ...
        document.getElementById('email_contact_selector').classList.toggle('hidden',obj.email != '');
    }
}