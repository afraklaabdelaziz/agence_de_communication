...
jQuery(function(){
	...
	jQuery('#zbsQuoteBuilderSendNotification').off('click').on( 'click', function(ind,ele){
...
function zbscrm_getTemplatedQuote(cb,errcb){
	...
	if (!window.quoteTemplateBlocker){
		...
		// req:
		var custID = jQuery('#zbscq_customer').length > 0 ? parseInt(jQuery('#zbscq_customer').val()) : -1;
		var quoteTemplateID = jQuery('#zbs_quote_template_id').length > 0 ? parseInt(jQuery('#zbs_quote_template_id').val()) : -1;
		...
		if (custID > 0){
			if (quoteTemplateID > 0){
				...
				quoteTemplateAJAX.done(function(e) {
					if (e.error == 1 || e.processed == -1) {
						...
						setTimeout(function(){ zbscrm_appendTextToEditor(e.html); },500);
						//if blank, auto-fill with template values
						if (jQuery('#title').length > 0 && jQuery('#title').val() == "") jQuery('#title').val(e.template_title);
						if (jQuery('#value').length > 0 && jQuery('#value').val() == "") jQuery('#value').val(e.template_value);
						if (jQuery('#notes').length > 0 && jQuery('#notes').val() == "") jQuery('#notes').val(e.template_notes);
			...
			swal({title: "Error!",text: "Please Choose a Customer",type: "error",confirmButtonText: "OK"});
			window.quoteTemplateBlocker = false;
			return false; 
		}
