...
<p id="sbDesc"><?php _e('Set up your global mail settings. This, along with Mail Delivery settings and Mail Templates, make up the backbone of the CRM system.',"zero-bs-crm"); ?></p>
    ...
    <form method="post" action="?page=<?php echo $zbs->slugs['settings']; ?>&tab=mail">
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                ...
                <td style="width:540px"><input type="text" class="winput form-control" name="wpzbs_unsub" id="wpzbs_unsub" value="<?php if (isset($settings['businessyourname']) && !empty($settings['unsub'])) echo $settings['unsub']; ?>" placeholder="<?php _e("e.g. You're seeing this because you're registered as a contact of Michael Scott Paper Company, if you'd like to unsubscribe from any future communications please click ##UNSUB-LINK##.", 'zero-bs-crm' ); ?>" /></td>
            </tr>
            <tr>
                <td class="wfieldname"><label for="wpzbscrm_unsubpage"><?php _e('Unsubscribe Page',"zero-bs-crm"); ?>:</label><br /><?php _e("Select the WordPress page with your unsubscribe shortcode (Required for Mail Campaigns).","zero-bs-crm");?>
                </td>
                <td>
                    <?php
