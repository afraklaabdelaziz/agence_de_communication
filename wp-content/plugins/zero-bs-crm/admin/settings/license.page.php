...
if (!zeroBSCRM_isLocal(true)){
    ...
    if (!is_array($license_key_array) || empty($license_key_array['key'])){
        ...
        ##WLREMOVE
        $msg .= ' ' . sprintf( __( 'Please visit <a href="%s" target="_blank">your account</a> for your CRM key and license management.', "zero-bs-crm" ), $zbs->urls['account'] );
        $msg .= '<br><br>';
        $msg .= sprintf(__("Don't have a license yet? Take a look at <a href='%s' target='_blank'>our extension bundles</a>!",'zero-bs-crm'), $zbs->urls['pricing']);
        ##/WLREMOVE
        echo $msg;
        echo "</div>";
        ...
        if (!$licenseValid){
            ...
            if (!empty($lastErrorMsg)){
                ...
                $msg .= '<br /><br />';
                $msg .= sprintf(__('If you believe you are seeing this in error, please <a href="%s" target="_blank">contact support</a> and share the following debug output:','zero-bs-crm'), $zbs->urls['support']);
            ...
            if (isset($license_key_array['extensions_updated']) && $license_key_array['extensions_updated'] === false){
                ...
                $msg .= '<br><br>';
                $msg .= ' '.sprintf( __( 'You have extensions which need updating. Please <a href="%s">go here</a> to update.', 'zero-bs-crm' ), admin_url( 'update-core.php' ) );
            ...
            // view license
            echo '<div class="four wide column" style="text-align:right;padding-top:1.5em;padding-right:2em"><span class="zbs-license-show-deets ui mini blue button" class="ui link"><i class="id card icon"></i> '.__('License details','zero-bs-crm').'</span></div>';
            echo '</div>'; // / grid
            ...
            if ( isset( $license_key_array['access'] ) ) {
                echo $zbs->getSubscriptionLabel( $license_key_array['access'] );
            }
            else {
                echo '-';
            }
            ...
            if ( isset( $license_key_array['expires'] ) && $license_key_array['expires'] > 0 ) {
                echo zeroBSCRM_locale_utsToDate( $license_key_array['expires'] );
            }
            else {
                echo '-';
            }
            ...
            echo '<br><br>';
            echo sprintf( __( 'As needed, please visit <a href="%s" target="_blank">your account</a> to manage your license keys and billing.', 'zero-bs-crm' ), $zbs->urls['account'] );
            ##/WLREMOVE
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                ...
                <td style="width:540px">
                    <input style="padding:10px;" name="wpzbscrm_license_key" id="wpzbscrm_license_key" class="form-control" type="text" value="<?php if (isset($license_key_array['key']) && !empty($license_key_array['key'])) echo $license_key_array['key']; ?>" />
                </td>
            </tr>
