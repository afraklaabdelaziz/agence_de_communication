...
$fieldTypes = array(
    ...
    'company' => array('name'=>jpcrm_label_company().' Fields','obj'=>'zbsCompanyFields'),
    ...
    <form method="post" action="?page=<?php echo $zbs->slugs['settings']; ?>&tab=fieldsorts" id="zbsfieldsortform">
        ...
        <?php foreach ($fieldTypes as $key => $fieldType){ ?>
            ...
            <table class="table table-bordered table-striped wtab">
                ...
                <tr>
                    <th class="wmid"><?php _e($fieldType['name'],"zero-bs-crm"); ?>:</th>
                </tr>
                                        ...
                                        if (isset($field['migrate']) && !empty($field['migrate'])){
                                            ...
                                            if (in_array($key,array('customer','company'))){
                                                ...
                                                if (isset($field['essential']) && !empty($field['essential'])){
                                                    ...
                                                    ?><div class="zbs-showhide-field"><label for="zbsshowhide<?php echo $key.'-'.$subkey; ?>"><?php _e('Hide',"zero-bs-crm"); ?>:</label><input id="zbsshowhide<?php echo $key.'-'.$subkey; ?>" type="checkbox" value="1"<?php if ($hidden) echo ' checked="checked"'; ?> /></div><?php
