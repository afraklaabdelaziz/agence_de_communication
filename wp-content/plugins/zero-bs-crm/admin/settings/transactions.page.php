...
if (isset($_POST['editwplf']) && zeroBSCRM_isZBSAdminOrAdmin()){
    ...
    $transactionStatusStr = ''; if (isset($_POST['jpcrm-status-transactions']) && !empty($_POST['jpcrm-status-transactions'])) $transactionStatusStr = sanitize_text_field($_POST['jpcrm-status-transactions']);

    if (strpos($transactionStatusStr, ',') > -1) {

        #} Trim them...
        $statusArr = array(); $statusUncleanArr = explode(',',$transactionStatusStr);
        foreach ($statusUncleanArr as $x) {
            $z = trim($x);
            if (!empty($z)) $statusArr[] = $z;
        }
        ...
        #} only 1? or empty?
        if (!empty($transactionStatusStr)) $customisedFields['transactions']['status'][1] = $transactionStatusStr;
...
<div id="sbA">
    ...
    <form method="post" action="?page=<?php echo $zbs->slugs['settings']; ?>&tab=transactions">
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                <th colspan="2" class="wmid"><?php _e('General Transaction Settings',"zero-bs-crm"); ?>:</th>
            </tr>
            ...
            <tr>
                <td width="94">
                    <label for="wpzbscrm_shippingfortransactions"><?php _e('Use Shipping',"zero-bs-crm"); ?></label>
                </td>
                <td>
            ...
            <tr>
                <td width="94">
                    <label for="wpzbscrm_paiddatestransaction"><?php _e('Use Paid/Completed Dates',"zero-bs-crm"); ?></label>
                </td>
                <td>
            ...
            <tr>
                <td class="wfieldname"><label><?php _e('Include these statuses in total value',"zero-bs-crm"); ?>:</label><br /><?php _e('Tick which statuses to include when calculating total transaction value and total overall value of contacts.',"zero-bs-crm"); ?>
                    <br /><br /></td>
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                <th class="wmid"><?php _e('Transaction Field Options',"zero-bs-crm"); ?>:</th>
            </tr>
            ...
            <tr>
                <td style="padding:2%;">
                    ...
                    <table class="table table-bordered table-striped wtab">
                        ...
                        <tr>
                            <td width="94">
                                <label for="jpcrm-status-transactions"><?php _e('Transaction Status',"zero-bs-crm"); ?></label>
                            </td>
                            <td>
                                <?php
                                ...
                                if (isset($settings['customisedfields']['transactions']['status']) && is_array($settings['customisedfields']['transactions']['status'])) $jpcrmTranStatusStr = $settings['customisedfields']['transactions']['status'][1];
                                if (empty($jpcrmTranStatusStr)) {
                                    #} Defaults:
                                    global $zbsTransactionFields; if (is_array($zbsTransactionFields)) $jpcrmTranStatusStr = implode(',',$zbsTransactionFields['status'][3]);
                                }

                                ?>
                                <input type="text" name="jpcrm-status-transactions" id="jpcrm-status-transactions" value="<?php echo $jpcrmTranStatusStr; ?>" class="form-control" />
                                <p style="margin-top:4px"><?php _e("Default is","zero-bs-crm");?>:<br /><span style="background:#ceeaea;padding:0 4px">Succeeded,Completed,Failed,Refunded,Processing,Pending,Hold,Cancelled,Deleted</span></p>
                            </td>
                        </tr>
        ...
        <table class="table table-bordered table-striped wtab">
            ...
            <tr>
                <th colspan="2" class="wmid"><?php _e('Additional settings on transactions',"zero-bs-crm"); ?>:</th>
            </tr>
            ...
            <tbody>
            <tr>
                <td class="wfieldname"><label for="wpzbscrm_transaction_fee"><?= __('Show fee',"zero-bs-crm") ?>:</label><br /><?= __('Tick if you need to use fees',"zero-bs-crm") ?></td>
                <td style="width:540px"><input type="checkbox" class="winput form-control" name="wpzbscrm_transaction_fee" id="wpzbscrm_transaction_fee" value="1"<?php if (isset($settings['transaction_fee']) && $settings['transaction_fee'] == "1") echo ' checked="checked"'; ?> /></td>
            </tr>
            <tr>
                <td class="wfieldname"><label for="wpzbscrm_transaction_tax"><?= __('Show tax',"zero-bs-crm") ?>:</label><br /><?= __('Tick if you need to use taxes',"zero-bs-crm") ?></td>
                <td style="width:540px"><input type="checkbox" class="winput form-control" name="wpzbscrm_transaction_tax" id="wpzbscrm_transaction_tax" value="1"<?php if (isset($settings['transaction_tax']) && $settings['transaction_tax'] == "1") echo ' checked="checked"'; ?> /></td>
            </tr>
            <tr>
                <td class="wfieldname"><label for="wpzbscrm_transaction_discount"><?= __('Show discount',"zero-bs-crm") ?>:</label><br /><?= __('Tick if you need to use discounts',"zero-bs-crm") ?></td>
                <td style="width:540px"><input type="checkbox" class="winput form-control" name="wpzbscrm_transaction_discount" id="wpzbscrm_transaction_discount" value="1"<?php if (isset($settings['transaction_discount']) && $settings['transaction_discount'] == "1") echo ' checked="checked"'; ?> /></td>
            </tr>
            <tr>
                <td class="wfieldname"><label for="wpzbscrm_transaction_net"><?= __('Show net amount',"zero-bs-crm") ?>:</label><br /><?= __('Tick if you need to use net amount',"zero-bs-crm") ?></td>
                <td style="width:540px"><input type="checkbox" class="winput form-control" name="wpzbscrm_transaction_net" id="wpzbscrm_transaction_net" value="1"<?php if (isset($settings['transaction_net']) && $settings['transaction_net'] == "1") echo ' checked="checked"'; ?> /></td>
            </tr>
            </tbody>
    ...
    <div style="text-align: center;margin-top:2.5em">
        <span class="ui label"><?php _e('Other Tools:','zero-bs-crm'); ?></span> <a href="<?php echo esc_url(zbsLink($zbs->slugs['settings'].'&tab=customfields#zbscrm-transactions-custom-fields')); ?>"><?php _e('Manage Custom Fields','zero-bs-crm'); ?></a>
    </div>
