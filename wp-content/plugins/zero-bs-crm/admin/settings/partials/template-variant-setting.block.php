...
if ( ! defined( 'ZEROBSCRM_PATH' ) ) exit; 
    ...
    global $zbs;
                ...
                if ( is_array( $variants ) && count( $variants ) > 0 ){ ?>              
                    <select id="<?php echo $setting_key; ?>" name="<?php echo $setting_key; ?>">
                        ...
                        foreach ( $variants as $template_path => $template_info ){
                            ...
                            ?><option value="<?php echo $option_value; ?>"<?php if ( $settings[$setting_key] == $option_value ) { echo ' selected="selected"'; } ?>>
                                <?php if ( $default ) _e( 'Default Template:', 'zero-bs-crm' ).' '; ?>
                                <?php echo $template_info['filename'].' ('.$template_info['origin'].')'; ?>
                            </option><?php
                        ...
                        if ( !$default_present ){
                            ...
                            ?><option value=""<?php if ( empty( $settings[$setting_key] ) ) { echo ' selected="selected"'; } ?>>
                                <?php echo __( 'Default Template: ', 'zero-bs-crm' ) . $template_file; ?>
                            </option><?php
                        }
