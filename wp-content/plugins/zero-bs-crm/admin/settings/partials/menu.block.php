...
<div class="ui vertical fluid menu" id="zbs-settings-menu">
    ...
    <?php foreach( $sortedTabs as $tab => $tabArr ): ?>
        ...
        <?php if ( isset( $tabArr['submenu'] ) ): ?>
            <div class="item">
                <a class='item zbs-settings-head <?= ( $tab == $current ) ? ' active' : '' ?>>' href='?page=<?= $zbs->slugs['settings'] ?>&tab=<?= $tab ?>'><?= $ico.$name ?></a>
                <div class="menu">
                    ...
                    <?php foreach ( $tabArr['submenu'] as $tab2 => $tabArr2 ): ?>
                        ...
                        <a class='item <?= $class ?>' href='<?= $url ?>'><?= $new.$ico2.$name2 ?></a>
        ...
        <?php else: ?>
            ...
            <a class='item <?= ( $tab == $current ) ? ' active' : '' ?>' href='?page=<?= $zbs->slugs['settings'] ?>&tab=<?= $tab ?>'><?= $ico.$name ?></a>

        <?php endif ?>

    <?php endforeach ?>
    <?php ##WLREMOVE ?>
    <a class="item" href="<?= zbsLink( $zbs->slugs['extensions'] ) ?>"><i class="ui orange puzzle piece icon"></i> <?= __( 'Extensions','zero-bs-crm' ) ?></a>
    <?php ##/WLREMOVE ?>
</div>