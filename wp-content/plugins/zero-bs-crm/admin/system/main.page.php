...
// retrieve active tab
if ( isset( $_GET['tab'] ) ) $active_tab = sanitize_text_field( $_GET['tab'] );

//set fallback tab
if ( !isset( $tabs[$active_tab] )) $active_tab = 'assistant';
...
<div id="jpcrm-system-manager" style="margin:1em;">
    ...
    <div class="ui top attached tabular menu">
        ...
        <?php foreach ($tabs as $k => $v): ?>
            <div data-tab="<?php echo $k ?>" class="<?php echo $active_tab == $k ? 'active ' : '' ?>item"><?php echo $v['tab_name'] ?></div>
        <?php endforeach; ?>

    </div>

    <?php foreach ($tabs as $k => $v): ?>
        <div class="ui bottom attached <?php echo $active_tab == $k ? 'active ' : '' ?>tab segment" data-tab="<?php echo $k ?>">
