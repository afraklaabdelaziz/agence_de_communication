...
function zeroBSCRM_isCustomerPortalDisabled($cID=-1){
		...
		if ($cID < 1) {
			...
			// return check
			global $zbs;
			return $zbs->DAL->contacts->getContactMeta($cID,'portal_disabled');
...
function zeroBS_getExternalContactSource($contactID = -1){
	...
	if ($contactID > 0){
		...
		$extSources = $zbs->DAL->contacts->getExternalSourcesForContact(array(
...
// returns ONLY THE FIRST external source.
...
// returns all external sources
function zeroBS_getExternalSources($objID=-1,$objType=-1){

	if ($objID > 0 && $objType > 0){

		global $zbs;
	
		$extSources = $zbs->DAL->getExternalSources(-1,array(
...
/* ======================================================
   ...
   function zeroBS___________CompanyHelpers(){return;}
	...
	function zeroBS_getCompanyExternalSource($cID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('CRM Function Deprecated in v3.0+. Please use zeroBS_getExternalSource()');
...
/* ======================================================
   ...
   function zeroBS___________InvoiceHelpers(){return;}
	...
	function zeroBSCRM_getNextInvoiceID(){
		...
		#} Retrieves option, and returns, is dumb for now.
		// DAL1+2: return (int)get_option('invoiceindx',$defaultStartingQuoteID)+1;
	...
	function zeroBSCRM_get_invoice_defaults($objID = -1){
	    ...
	    $defaults = array(
	            ...
	            'new_invoice'               => true,
	            // these was legacy from <v3 and firstfix v3 invoicing. Use id + id_override fields only, v3+
	            ...
	            // dates now match the invObj
	            'date_date'                 => $default_date,                        //need to sort this out on the way out (in TS to outputtable date for Date Picker)
	            'date' 						=> time(),
	            'due'                       => 0,

	            'hash'                      => zeroBSCRM_hashes_GetHashForObj($objID,ZBS_TYPE_INVOICE),
	            'logo_url'			        => $logo, // invoice_logo_url
	            'bill'                      => '',
	            'bill_name'                      => '',
	            'settings'                  => $settings,
	...
	// was called zeroBSCRM_getInvoiceData -> zeroBSCRM_invoicing_getInvoiceData
	 function zeroBSCRM_invoicing_getInvoiceData($invID=-1){
	    ...
	    if ($invID > 0){
	        ...
	        // <DAL3 $invoice = get_post_meta($invID, 'zbs_customer_invoice_meta', true);
	        ...
	        if (!is_array($invoice)){
	            ...
	            if (!isset($invoice['date'])){
	                // <DAL3 $invoice['date'] = get_the_date('',$invID);
	    			$invoice['date'] = zeroBSCRM_date_i18n(-1,time(),false,true);
	            }
	            ...
	            $data['invoiceObj'] = $invoice;
	                ...
	                #} Hash
	                // <DAL3 $hash = zeroBSCRM_hashes_GetHashForObj($invID,ZBS_TYPE_INVOICE);

	                // catch any empty shiz? seems to be what was happening.
	                if (!isset($data['invoiceObj']['invoice_logo_url'])) $data['invoiceObj']['invoice_logo_url'] = $settings['logo'];
	                
	                // these was legacy from <v3 and firstfix v3 invoicing. Use id + id_override fields only, v3+
	                //$data['invoiceObj']['invoice_id']                   = $invID;
	                //$data['invoiceObj']['invoice_custom_id']            = $invoice['id_override']; //get_post_meta($invID,'zbsid', true);
	                
	                // these two are kind of just aliases? Use straight $invoiceObj[contact] etc.
	                $data['invoiceObj']['invoice_contact']              = false; //get_post_meta($invID, 'zbs_customer_invoice_customer', true);
	                if (isset($invoice['contact']) && is_array($invoice['contact']) && count($invoice['contact']) > 0) $data['invoiceObj']['invoice_contact'] = $invoice['contact'][0];
	                $data['invoiceObj']['invoice_company']              = false; //get_post_meta($invID, 'zbs_company_invoice_company', true);
	                if (isset($invoice['company']) && is_array($invoice['company']) && count($invoice['company']) > 0) $data['invoiceObj']['invoice_company'] = $invoice['company'][0];
	                $data['invoiceObj']['new_invoice']                  = false;

	                // <DAL3 $data['invoiceObj']['status']                       = strtolower($invoice['status']);
	                // <DAL3 $data['invoiceObj']['ref']                          = $invoice['ref'];
	                ...
	                if (isset($data['invoiceObj']['invoice_contact']) && is_array($data['invoiceObj']['invoice_contact']) && isset($data['invoiceObj']['invoice_contact']['id'])){
	                	...
	                	if (isset($data['invoiceObj']['invoice_contact']['email'])) $billing_email = $data['invoiceObj']['invoice_contact']['email'];
	                	if (isset($data['invoiceObj']['invoice_contact']['name'])) $billing_name = $data['invoiceObj']['invoice_contact']['name'];
	                    ...
	                    if (isset($data['invoiceObj']['invoice_company']['email'])) $billing_email = $data['invoiceObj']['invoice_company']['email'];
                      if (isset($data['invoiceObj']['invoice_company']['name'])) $billing_name = $data['invoiceObj']['invoice_company']['name'];
                  ...
                  //add billing name here
	                ...
	                // <DAL3 $data['invoiceObj']['invoice_logo_url']             = $logo;
	                // <DAL3 $data['invoiceObj']['hash']                         = $hash;
	                // <DAL3 $data['invoiceObj']['date']                         = $invoice['date'];     

	                // needs translating
	                $hoursOrQuantity = (int) $invoice['hours_or_quantity']; // 0 = hours, 1 = quantity
	                $hoursOrQuantityStr = 'hours'; if ($hoursOrQuantity > 0) $hoursOrQuantityStr = 'quantity';
	                $data['invoiceObj']['invoice_hours_or_quantity']    = $hoursOrQuantityStr; // get_post_meta($invID, 'zbsInvoiceHorQ', true);
	                ...
	                if ($data['invoiceObj']['portal_installed']){
		                ...
		                $portalLink         = zeroBS_portal_link(); if (substr($portalLink, -1) !== '/') $portalLink .= '/';
                    ...
                    */
	                ...
	                // DAL3 + these are just "associated trans" (can be partials, full payments, credit notes etc.)
...
/* ======================================================
   ...
   ====================================================== */
	...
	function zeroBSCRM_storeAPIKey($api_key=''){
		...
		global $zbs; 
		return $zbs->DAL->updateSetting('api_key',$api_key);
	...
	function zeroBSCRM_updateAPIKey($api_key=''){
		...
		return zeroBSCRM_storeAPIKey($api_key);
	...
	function zeroBSCRM_getAPIKey(){
	    ...
	    global $zbs; return $zbs->DAL->setting('api_key');

	}

	function zeroBSCRM_getAPIKeys(){

	    // for now is just a wrapper really...
	    return zeroBSCRM_getAPIKey();
	...
	function zeroBSCRM_storeAPISecret($api_secret=''){
		...
		global $zbs; return $zbs->DAL->updateSetting('api_secret',$api_secret);
	...
	function zeroBSCRM_getAPISecret(){
	    ...
	    global $zbs; return $zbs->DAL->setting('api_secret');
...
/* ======================================================
   ...
   ====================================================== */
    ...
    // use zeroBS_getObjOwnerWPEmail in future... (renamed, bad naming)
