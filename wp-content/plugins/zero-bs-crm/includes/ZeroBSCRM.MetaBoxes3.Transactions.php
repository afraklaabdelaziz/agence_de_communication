        ...
        public function html( $transaction, $metabox ) {
	    		...
	    		<table class="form-table wh-metatab wptbp" id="wptbpMetaBoxMainItem">
					...
					<tr class="wh-large">
		                ...
		                <td>
		                	<input type="text" name="zbst_date" id="transactionDate" class="form-control zbs-date-time" placeholder="" value="<?php if(isset($transaction['date'])){

								#} Put in format (inc time from 3.0+)
								echo zeroBSCRM_date_i18n_plusTime(-1,$transaction['date'],true);
			        ...
			        <?php #} Custom Fields
	                    ...
	                    // wh centralised 20/7/18 - 2.91+
	                    	...
	                    	// v3, ref = orderid, title = item, 
		                	// TODO resolve zbsTransactionFields global for new DAL
		                	$skipList = array('ref','customer','status','total','customer_name','date','currency','title','tax_rate','taxes');
        ...
        public function save_data( $transactionID, $transaction ) {
            ...
            if (!defined('ZBS_T_SAVED')){
                ...
                if (empty($transactionID) || $transactionID < 1)  $transactionID = -1;
					...
					$transaction['date_completed'] 	= empty( $transaction['date_completed'] ) ? $transaction['date'] : $transaction['date_completed'];
					$transaction['date_paid'] = empty( $transaction['date_paid'] ) ? $transaction['date'] : $transaction['date_paid'];
