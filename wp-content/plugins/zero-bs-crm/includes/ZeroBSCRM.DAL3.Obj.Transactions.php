...
class zbsDAL_transactions extends zbsDAL_ObjectLayer {
     ...
     * @return array 
     */
    function getTransactionStatusesToIncludeQuery(){


        // first we get the setting
        $transStatuses = $this->getTransactionStatusesToInclude(); 
        ...
        $queryAddition = ''; 
        if (is_array($transStatuses)){

            // create escaped csv
            $transStatusesStr = $this->build_csv($transStatuses);

            // build return sql
            $queryAddition = ' AND zbst_status IN ('.$transStatusesStr.')';
