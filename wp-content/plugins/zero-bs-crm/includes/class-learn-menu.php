	...
	public function render_generic_learn_menu(
	    ...
	    <div id="zbs-admin-top-bar" style="margin-bottom:-20px;">
	      <div id="zbs-list-top-bar">
	          <div class="zbs-white"><span class="jpcrm-learn-page-title"><?php echo $page_title; ?></span><?php 
	          if ( !empty( $add_new ) ) {
	          	echo ' '.$add_new;
	          } ?>
	          	<?php if ($show_learn){ 
					...
					<div class="ui special popup top left transition hidden" id="learn-pop" style="<?php echo esc_attr( $popup_extra_css ); ?>">
						...
						<div class="jpcrm-learn-container ui grid">
							...
							?> wide column">
								<h3 class="learn-h3"><?php echo $learn_title; ?></h3>
								<div class="content">
									<?php 
									...
									if ( !empty( $learn_more_url ) ){
										echo '<br/><a href="' . esc_url( $learn_more_url ) . '" target="_blank" class="learn-more-link">' . __( "Learn More", 'zero-bs-crm' ) . '</a>';
									} ?>
								</div>
							</div>
								...
								<?php
									...
									if ( !empty( $learn_video_url ) ){
										...
										<div class="jpcrm-learn-popup-video">
											...
											<a href="<?php echo $learn_video_url; ?>" target="_blank"><img src="<?php echo $video_thumbnail_url; ?>" alt="<?php echo $learn_video_title; ?>" class="jpcrm-video-thumbnail" /></a>
											<?php } ?>
						                    <br>
						                    <?php 
						                    	...
						                    	if ( !empty( $learn_video_title ) ){ 
						                    		?><a href="<?php echo $learn_video_url; ?>" target="_blank"><?php echo $learn_video_title; ?></a><?php
						                		}

							                ?><div class="jpcrm-learn-popup-video-cta">
							                	<a href="<?php echo $zbs->urls['youtube_intro_playlist']; ?>" target="_blank"><img src="<?php echo ZEROBSCRM_URL; ?>i/first-use-dash-learn-video-ico.png" alt="<?php _e( "View More on YouTube", 'zero-bs-crm' ); ?>" /> <?php _e( "View More on YouTube", 'zero-bs-crm' ); ?></a>
							                </div>
										...
										<div class="jpcrm-learn-popup-image">
											<a href="<?php echo $learn_more_url; ?>" target="_blank"><img src="<?php echo $learn_image_url; ?>" alt="<?php echo $learn_title; ?>" class="jpcrm-learn-image" /></a>											
											<?php 

						                    	// show title if present
						                    	if ( !empty( $learn_title ) ){ 
						                    		?><br><a href="<?php echo $learn_more_url; ?>" target="_blank"><?php echo $learn_title; ?></a><?php
						                		}

							                ?>
						                    <div class="jpcrm-learn-popup-image-cta">
							                	<a href="<?php echo $learn_more_url; ?>" target="_blank"><div class="jpcrm-learn-popup-external-link"></div> <?php _e( "Read more", 'zero-bs-crm' ); ?></a>
							                </div>
						                </div><?php
	...
	private function get_all_content() {
		...
		$learn_menu_array = array(
		  ...
		  'editform' => array(
            ...
            'video_title' => __( "All about Forms", 'zero-bs-crm' ),
		    'url' => 'https://jetpackcrm.com/feature/forms/',
			'add_new' => ' <a href="' . zbsLink( 'create', -1, 'zerobs_form', false ) . '" class="button ui blue tiny zbs-add-new">' . __( 'Add New',"zero-bs-crm") . '</a>'
		  ),
		  'formnew' => array(
		  ...
		  'quoteedit' => array(
			...
			'add_new' => '<div id="zbs-quote-learn-nav"></div>  <a href="' . esc_url(zbsLink('create',-1,ZBS_TYPE_QUOTE,false)) . '" class="button ui blue tiny zbs-add-new">' . __('Add New',"zero-bs-crm") . '</a>'
		  ),
		  'quotetags' => array(
		  ...
		  'quote-templates' => array(
			...
			'add_new' => ' <a href="' . zbsLink('create',-1,'zerobs_quo_template',false)  . '#free-extensions-tour" class="button ui blue tiny zbs-add-new" id="add-template">' . __('Add Template',"zero-bs-crm") . '</a>'
		  ),
		  ...
		  'notifications' => array(
		    ...
		    'url' => 'https://kb.jetpackcrm.com/knowledge-base/zero-bs-notifications/',
		  ...
		  'exportcontact' => array(
			...
			'add_new' => '<a href="' . admin_url( 'admin.php?page=' . $zbs->slugs['zbs-export-tools'] . '&zbswhat=contacts' ) . '" class="button ui blue tiny zbs-add-new">' . __( 'Export Other Types',"zero-bs-crm") . '</a>'
		  ),
		  'export-tools' => array(
		  	'title' => __( "Export Tools", 'zero-bs-crm' ),
		    'url' => 'https://jetpackcrm.com/feature/tools/',
		  ...
		  'bulktools' => array(
		  	'title' => __( "Bulk Tools", 'zero-bs-crm' ),
		    'url' => 'https://jetpackcrm.com/feature/tools/',
		    'img' => 'learn-bulk-tools.png',
		    'video' => false,
		    'content' => "<p>" . __( "Bulk Tools let you run bulk deletion routines. Remove all contacts imported from a particular tool or service.", 'zero-bs-crm' ) . "</p><p>" . __( "If you want to reset your whole CRM please contact support.", 'zero-bs-crm' ) . "</p>",
		  ),
		  'datatools' => array(
		  ...
		  'migratedb2contacts' => array(
		  	'title' => __( "Database Upgrade", 'zero-bs-crm' ),
		    'url' => 'https://kb.jetpackcrm.com/knowledge-base/updating-contact-database-dbv2-migration/',
		    'img' => 'learn-extensions-list.png',
		    'video' => false,
		    'content' => "<p>" . __( "Jetpack CRM Forced updates are rare, but when they happen, they come with a pay off!", 'zero-bs-crm' ) . "</p><p>" . __( "Running this Update routine will migrate your old contact data into the new database format, which has shown to be something in the order of 20x faster!", 'zero-bs-crm' ) . "</p>",
		  ),
		  'connect' => array(
		  ...
		  'modules' => array(
		    ...
		    'url' => 'https://jetpackcrm.com/feature/diy-crm/',
		  ...
		  'profile' => array(
		    ...
		    'url' => 'https://jetpackcrm.com/feature/your-crm',
		    'img' => 'learn-your-profile.png',
		    'video' => false,
		    'content' => "<p>" . __( "This is your profile page. It gives you useful information about your CRM and what you have been doing in it.", 'zero-bs-crm' ) . "</p><p>" . __( "It is also the place where you can connect your online Calendar to the CRM and have it show up in the Task Scheduler Calendar.", 'zero-bs-crm' ) . "</p>",
		  ),
		  'formedit' => array(
		    'img' => 'learn-forms.png',
		    'video' => false,
		  ...
		  'feedback' => array(
		    ...
		    'img' => 'learn-feedback.png',
		    'video' => false,
		    'content' => "<p>" . __( "Feedback helps us to improve this CRM for you and other users. We\'ve built almost all of what you can see in Jetpack CRM based on users feedback.", 'zero-bs-crm' ) . "</p><p>" . __( " We read and intergrate all feedback into our future development. <strong>So please do send us your Feedback</strong>.", 'zero-bs-crm' ) . "</p>",
		  ),
		  'delete' => array(
