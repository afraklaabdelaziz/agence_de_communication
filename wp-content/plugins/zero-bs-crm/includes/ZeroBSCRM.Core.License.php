...
function zeroBSCRM_admin_nag_footer(){
	...
	if($zbs->extensionCount(true) > 0) {
		...
		if ( get_transient('jpcrm-license-modal') ) return;
			...
			if ( zeroBSCRM_isAdminPage() && ((isset($_GET['page']) && $_GET['page'] != 'zerobscrm-plugin-settings') || (!isset($_GET['page']))) ){
				...
				if (isset($license) && !empty($license)){
					...
					if((isset($license['key']) && $license['key'] == '') || !isset($license['key'])){ 
						...
						$message .= '<p><a href="'.$zbs->urls['licensekeys'].'" class="ui button green" target="_blank">'.__('Retrieve License Key','zero-bs-crm').'</a>&nbsp;&nbsp;&nbsp;<a href="'.esc_url(zbsLink($zbs->slugs['settings']).'&tab=license').'" class="ui button blue">'.__('License Settings','zero-bs-crm').'</a></p>';
						
						// output modal
						zeroBSCRM_show_admin_nag_modal($message); return;
...
function zeroBSCRM_show_admin_nag_modal($message = ''){
	...
	if ( !get_transient('jpcrm-license-modal') ){
		...
		?><script type="text/javascript">var jpcrm_modal_message_licensing_nonce = '<?php echo wp_create_nonce( "jpcrm-set-transient-nonce" ); ?>';</script>
		<div class="zbs_overlay" id="jpcrm-modal-message-licensing">
			...
			<div class='zbs-message-body'>
				<img style="max-width:350px;margin-bottom:1.4em" src="<?php echo jpcrm_get_logo(false,'white'); ?>" alt="" style="cursor:pointer;" />
