...
function zeroBSCRM_mailDelivery_accountDDL($selectedOption=-1,$id='zbs-mail-delivery-acc',$showProtocol=true,$withSemanticWrap=true){
	...
	if (count($zbsSMTPAccs) <= 0){
		...
		// none
		?><select class="fluid" id="<?php echo $id; ?>" name="<?php echo $id; ?>"><option value="-1"><?php echo $defaultDetails; ?></option></select><?php
...
function zeroBSCRM_mailDelivery_commonSMTPSettings(){
	return array(

				// auth: tls, ssl, none

		'ses1' => array(
				'name' => 'AWS SES US East (N. Virginia)',
...
function zeroBSCRM_mailDelivery_retKeyData($str=''){
	if ( ! function_exists('zeroBSCRM_decrypt' ) ) {
		require( ZEROBSCRM_INCLUDE_PATH . 'ZeroBSCRM.Encryption.php' );
	}

	#} Unencrypt PW:
	$key = hex2bin( zeroBSCRM_getSetting('smtpkey') );
	$unenc = zeroBSCRM_decrypt( $str, $key );
	return $unenc;
}
