...
?>
    ...
    <div id="zbs-admin-top-bar">
      <div id="zbs-list-top-bar">
          <div class="zbs-white"><span class="add-new-button"><?php _e('CSV Importer Lite',"zero-bs-crm"); if (!empty($subpage)) echo ': '.$subpage; ?></span>
            ...
            <div class="ui special popup top left transition hidden" id="learn-pop">
              ...
              <div class="content">
				...
				##WLREMOVE
				?>
                <a href="<?php echo $zbs->urls['kbcsvformat']; ?>" target="_blank" class="ui button orange"><?php _e("Learn More","zero-bs-crm");?></a>
				<?php
				##/WLREMOVE
				?>
			  </div>
	...
	#} Validation (pre stage load)
		...
		if ($stage == 1){
			...
			if (!$nonceOkay) {
				...
				#} Send back to start
				$stage = 0; 
				$stageError = __( 'There was an error uploading your CSV file. Please try again.', 'zero-bs-crm' );
			}
			...
			if ( $movefile && ! isset( $movefile['error'] ) ) {
				...
				#} Check valid filetype:
					...
					if (strtolower($extension) != "csv"){
			    		...
			    		#} Send back to start
						$stage = 0; 
						$stageError = __( 'Your file is not a ".csv" file. If you are having continued problems please email support with a copy of your CSV file attached.', 'zero-bs-crm' );
					...
					if ($fileDetails['type'] != "text/csv" && $fileDetails['type'] != "text/plain") {
			    		...
			    		#} Send back to start
						$stage = 0; 
						$stageError = __( 'Your file is not a correctly formatted CSV, please check your file format. If you are having continued problems please email support with a copy of your CSV file attached.', 'zero-bs-crm' );

					}

					// Check format
					$fullCSV = file_get_contents($fileDetails['file']);
					$fullCSV = strip_tags($fullCSV);
					...
					if (count($csvLines) <= 0){
			    		...
			    		#} Send back to start
						$stage = 0; 
						$stageError = __( 'Your file does not appear to be a correctly formatted CSV File. We did not find any usable lines to import. If you are having continued problems please email support with a copy of your CSV file attached.', 'zero-bs-crm' );
		...
		if ($stage == 2){
			...
			if (!isset($_POST['zbscrmcsvimpf']) || !file_exists($_POST['zbscrmcsvimpf'])){
			    ...
			    #} Send back to start
				$stage = 0;
				$stageError = __( 'There was an error maintaining a link to your uploaded CSV file. Please contact support if this error persists, quoting error code #457', 'zero-bs-crm' ); #} lol.
			...
			if ($realFields == 0) $stageError = __( 'No fields were matched. You cannot import customers without at least one field mapped to a customer attribute.', 'zero-bs-crm' );
		...
		if ($stage == 3){
			...
			if ($realFields == 0) {
				...
				#} Back
				$stage = 0;
				$stageError = __( 'No fields were matched. You cannot import customers without at least one field mapped to a customer attribute.', 'zero-bs-crm' );
		...
		case 1:
			...
			?><div class="zbscrm-csvimport-wrap">
				...
				<div class="zbscrm-csv-map">
					<p class="zbscrm-csv-map-help"><?php _e('Your CSV File has been successfully uploaded. Before we can complete your import, you\'ll need to specify which field in your CSV file matches which field in your CRM.<br />You can do so by using the drop down options below:','zero-bs-crm'); ?></p>
   								...
   								<div class="zbscrm-csv-map-field">
   									...
   									<div class="zbscrm-csv-map-zbs-field">
   										<span class="to"><?php esc_html_e('To:','zero-bs-crm'); ?></span> <select name="zbscrm-csv-fieldmap-<?php echo $indx; ?>" id="zbscrm-csv-fieldmap-<?php echo $indx; ?>">
	   										...
	   										<option value="<?php echo $fieldID; ?>"><?php _e($fieldTitle,'zero-bs-crm'); ?></option>
	   										<?php } ?>
	   									</select>
	   								</div>
   								</div>
				...
				<div class="zbscrm-confirmimport-csv">
					...
					<form method="post" enctype="multipart/form-data" class="zbscrm-csv-import-form">
						...
						<?php } ?>   						
	   						...
	   						foreach ($fieldMap as $fieldID => $fieldTarget){
								...
								<input type="hidden" id="zbscrm-csv-fieldmap-<?php echo ($fieldID-1); ?>" name="zbscrm-csv-fieldmap-<?php echo ($fieldID-1); ?>" value="<?php echo $fieldTarget; ?>" />
	   							<div class="zbscrm-impcsv-map">
	   								<div class="zbscrm-impcsv-from"><?php if (!empty($fromStr)) echo '"'.$fromStr.'"'; else echo sprintf( __( 'Field #%s', 'zero-bs-crm' ), $fieldID ); ?></div>
	   								<div class="zbscrm-impcsv-arrow"><?php if ($fieldTarget != "ignorezbs") echo '<i class="fa fa-long-arrow-right"></i>'; else echo '-'; ?></div>
	   								<div class="zbscrm-impcsv-to"><?php if ($fieldTarget != "ignorezbs") echo '"'.$fieldTargetName.'"'; else _e( 'Ignore', 'zero-bs-crm' ); ?></div>
	   							</div><?php
   						...
   						<hr />
						<div style="text-align:center">
							<button type="submit" name="csv-map-submit" id="csv-map-submit" class="button button-primary button-large" type="submit"><?php _e( 'Run Import', 'zero-bs-crm' ); ?></button>	
						</div>
   						<?php } else { 
   							#} No fields? wtf?
			...
			?><div class="zbscrm-csvimport-wrap">
				...
				<div class="zbscrm-final-stage">
					<div class="zbscrm-import-log">
						...
						<div class="zbscrm-import-log-line"><?php echo sprintf( __("Beginning Import of %s rows...","zero-bs-crm"), zeroBSCRM_prettifyLongInts( count( $csvLines ) ) );?></div>
						<?php 
							...
							if (count($csvLines) > 0) foreach ($csvLines as $line){
								...
								if ($lineIndx == 0 && $ignoreFirstLine){
									...
									echo '<div class="zbscrm-import-log-line">' . __( 'Skipping header row...', 'zero-bs-crm' ) . '<i class="fa fa-check"></i></div>';
									...
									#} split
	   								...
	   								if (count($customerFields) > 0){
	   									...
	   									if (!empty($newCustID) && empty($potentialCustomerID)) {
		   									...
		   									#} Line
											echo '<div class="zbscrm-import-log-line">'.esc_html(
																										sprintf(
							...
							if (count($existingOverwrites) > 0) {
								...
								echo '<div class="zbscrm-import-log-line"><strong>' . __( 'The following customers were already in your Jetpack CRM, and were updated:', 'zero-bs-crm' ) . '</strong></div>';
							...
							if (!$saveCopyOfCSVFile){
								...
								echo '<div class="zbscrm-import-log-line">' . __( 'CSV Upload File Deleted...', 'zero-bs-crm' ) . '<i class="fa fa-check"></i></div>';
				...
				<div class="zbscrm-upload-csv">
					...
					<form method="post" enctype="multipart/form-data" class="zbscrm-csv-import-form">
						<input type="hidden" id="zbscrmcsvimpstage" name="zbscrmcsvimpstage" value="1" />
   						<?php wp_nonce_field( 'zbscrm_csv_import', 'zbscrmcsvimportnonce' ); ?>
						<label class="screen-reader-text" for="zbscrmcsvfile"><?php _e(".CSV file","zero-bs-crm");?></label>
			  ...
			  $bundle = false; if ($zbs->hasEntrepreneurBundleMin()) $bundle = true;
			  	...
			  	if (!$bundle){
					...
					<div class="zbscrm-lite-notice">
						...
						<p><?php echo sprintf( __( 'If you would like to benefit from more features (such as logging your imports, automatically creating companies (B2B), and direct support) then please purchase a copy of our <a href="%s" target="_blank">CSV Importer PRO</a> extension.', 'zero-bs-crm' ), $zbs->urls['extcsvimporterpro'] ); ?><br /><br /><a href="<?php echo $zbs->urls['extcsvimporterpro']; ?>" target="_blank" class="ui button blue large"><?php _e("Get CSV Importer PRO","zero-bs-crm");?></a></p>
					...
					<div class="zbscrm-lite-notice">
						<h2><?php _e("CSV Importer: Lite Version","zero-bs-crm");?></h2>
						<p><?php echo sprintf( __( 'You have the PRO version of CSV importer available as part of your bundle. Please download and install from <a href="%s" target="_blank">your account</a>.', 'zero-bs-crm'), $zbs->urls['account'] ); ?></p>
					</div><?php
				}
			##/WLREMOVE
