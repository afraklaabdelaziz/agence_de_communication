...
const ENTREPRENEUR_YEARLY_PRICE = 199;
    ...
    <table class="ui fixed single line celled table" id="zbs-team-user-table">
      <tbody>
        ...
        foreach($zbsRoleIDs as $ID){
            ...
            echo "<tr><td>".$ID."</td><td>" . jpcrm_get_avatar( $ID, 30 ) . "<div class='dn'>" . $user->display_name . "</div></td><td>" . $caps_output . "</td>";
...
function zeroBSCRM_pages_edit_file(){
  ...
  global $zbs;
    ...
    if(isset($_POST['save']) && $_POST['save'] == -1){
      ...
      echo "<div class='ui message blue' style='margin-right:20pz'><i class='icon info'></i> ".__("Details saved", 'zero-bs-crm') ."</div>";
    ...
    <div class = "ui segment zbs-cp-file-edit-page">
      ...
      ?>
      <h4><?php _e("Edit File Details", 'zero-bs-crm'); ?></h4>
      <p>
        <?php _e("You are editing details for the following file", 'zero-bs-crm'); ?>
        <br/>
        <em><?php echo $file; ?>
        (<a href="<?php echo $ourFile['url']; ?>" target="_blank"><?php _e("View file", 'zero-bs-crm'); ?></a>)</em>
      </p>
      <form class="ui form" method="POST" action="#">

          <label for="title"><?php _e("Title", 'zero-bs-crm');?></label>
          <input class="ui field input" id="title" name="title" value="<?php if(isset($ourFile['title'])) echo $ourFile['title'];?>" />

          <label for="desc"><?php _e("Description", 'zero-bs-crm'); ?></label>
          ...
          <label for="fileportal"><?php _e("Show on Client Portal", 'zero-bs-crm'); ?></label>
          <select class="ui field select" id="fileportal" name="fileportal">
              <option value="0" <?php if(isset($ourFile['portal']) && $ourFile['portal'] == 0 ) echo "selected"; ?>><?php _e("No", 'zero-bs-crm'); ?></option>
              <option value="1" <?php if(isset($ourFile['portal']) && $ourFile['portal'] == 1 ) echo "selected"; ?>><?php _e("Yes", 'zero-bs-crm'); ?></option>
          </select>
          <?php } else { 
            ...
            if (current_user_can('admin_zerobs_manage_options') && $customer > 0){ ?>
              <label><?php _e("Show on Client Portal", 'zero-bs-crm'); ?></label>
          ...
          <?php
            if ($customer > 0){
              ...
              if (count($fileSlots) > 0){
                ...
                ?><label for="fileslot"><?php _e("Assign to Custom File Upload Box", 'zero-bs-crm'); ?></label>
                <select class="ui field select" id="fileslot" name="fileslot">
                  <option value="-1"><?php _e("None", 'zero-bs-crm');?></option><?php
          ...
          <input type="submit" class="ui button blue" value="<?php _e("Save details", 'zero-bs-crm'); ?>"/>
...
/* ======================================================
  ...
  function zeroBSCRM_pagination( $args = array() ) {
      ...
      if ( $firstpage && (1 != $page) )
          $echo .= '<li class="previous"><a href="' . $firstpage . '">' . __( 'First', 'zero-bs-crm' ) . '</a></li>';
      if ( $previous && (1 != $page) )
          $echo .= '<li><a href="' . $previous . '" title="' . __( 'previous', 'zero-bs-crm' ) . '">' . $args['previous_string'] . '</a></li>';
      ...
      if ($next && ($count != $page) )
          $echo .= '<li><a href="' . $next . '" title="' . __( 'next', 'zero-bs-crm' ) . '">' . $args['next_string'] . '</a></li>';
      
      $lastpage = esc_attr( zeroBSCRM_pagelink($count) );
      if ( $lastpage ) {
          $echo .= '<li class="next"><a href="' . $lastpage . '">' . __( 'Last', 'zero-bs-crm' ) . '</a></li>';
      }
      if ( isset($echo) )
      ...
      <div class="ui segment" style="margin-right:20px;">
        ...
        <div class="ui segment">
          ...
          <?php 
            ...
            <br /><a href="<?php echo $zbs->urls['kbtemplatefiles']; ?>" target="_blank" class="ui basic blue small button"><?php _e( 'Read more about templating', 'zero-bs-crm' ); ?></a>
            <?php 
            ##/WLREMOVE
            ?>
          </p>
          <div class="ui segment">
            <p><div class="ui label"><?php echo __( 'Current template file:', 'zero-bs-crm' ); ?></div></p>
            <p>&nbsp;&nbsp;&nbsp;<code><?php
              ...
              if ( is_array( $variants )  && isset( $variants['emails/default-email.html'] ) ){
                ...
                echo $variants['emails/default-email.html']['full_path'];
                $origin = $variants['emails/default-email.html']['origin'];

              } else {

                // default
                echo ZEROBSCRM_PATH . 'templates/emails/default-email.html'; 

              }

             ?></code></p>
             <p><div class="ui label"><?php echo __( 'Origin:', 'zero-bs-crm' ) . '</div> ' . $origin; ?></p>
          </div>
        </div>
                  ...
                  if(isset($_POST['zbssubject']) && !empty($_POST['zbssubject'])){
                    ...
                    */
                    ...
                    if(isset($_GET['zbs_template_id'])){
                      ...
                      echo "<div class='ui message green' style='margin-top:45px;margin-right:15px;'>" . __('Template updated', 'zero-bs-crm') . '</div>';
                        ...
                        if(isset($_GET['sendtest']) && !empty($_GET['sendtest'])){
                          ...
                          */
                          ...
                          if (!isset($mailDeliveryMethod) || empty($mailDeliveryMethod)) $mailDeliveryMethod = -1;
                            ...
                            echo "<div class='ui message green' style='margin-top:45px;margin-right:15px;'>" . __('Test Email Sent to ', 'zero-bs-crm') . '<b>'. $test_email .'</b></div>';
                        }
                          ...
                          echo '<div class="field">';
                            echo '<label for="zbssubject">' . __('Subject','zero-bs-crm') .'</label>';
                          ...
                          echo '<div class="field">';
                            ...
                            echo '<label for="zbs-mail-delivery-acc">' . __('Delivery Method','zero-bs-crm') .'</label>';
                          ...
                          */
                          ...
                          echo '<div class="field">';
                            echo '<label for="zbsbccto">' . __('Bcc To','zero-bs-crm') .'</label>';
                            echo '<input id="zbsbccto" name="zbsbccto" type="text" value="'.$form->zbsmail_bccto.'">';
                          echo '</div>';

                          echo '<div class="field">';
                            echo '<div class="ui grid">';
                              echo '<div class="eight wide column"><label class="jpcrm-email-template-label jpcrm-email-template-label-content" for="zbscontent">' . __('Content','zero-bs-crm') .'</label></div>';
  ...
  */
  ...
  <div id="zbs-welcome">
    <div class="container">
      ...
      <div class="intro">
          ...
          <div class="block" style="text-align:center;margin-top:-50px;">
						<img src="<?php echo jpcrm_get_logo(false) ?>" alt="Jetpack CRMt" id="jetpack-crm-welcome" style="text-align:center;padding:30px;"> 
      ...
      <div id="first-customer">
        <a href="https://jetpackcrm.com/learn/" target="_blank"><img src="<?php echo plugins_url('/i/first-customer-welcome-image.png', ZBS_ROOTFILE); ?>" alt="<?php _e('Adding your first customer','zero-bs-crm') ?>"></a>
      </div>
      ...
      <div id="action-buttons" class='block'>
        ...
        <div class='zbs-button-wrap'>
          ...
          <div class="right">
            <a href="<?php echo $zbs->urls['kbfirstcontact']; ?>" target="_blank" class='read-full-guide btn btn-hta'><?php _e("Read the full guide","zero-bs-crm");?></a>
          </div>
          <div class="clear"></div>
        </div>
      </div>
	...
	<div class="container margin-top30">
		<div class="intro">
			<div class="block" style="text-align:center;margin-top:-50px;">
				<img src="<?php echo plugins_url( '/i/ext/woocommerce-logo-horizontal-black.png', ZBS_ROOTFILE ); ?>" alt="WooCommerce" id="woocommerce-logo" style="text-align:center;padding:30px;padding-bottom:15px;max-width:440px;">
				<h6><?php _e( 'Jetpack CRM is WooCommerce ready!', 'zero-bs-crm' ); ?></h6>
			</div>
		</div>

      <div id="action-buttons" class='block'>
        <h6><?php _e("Connect today your WooCommerce Store and start importing your orders and customers into Jetpack CRM.","zero-bs-crm"); ?></h6>
        <div class='zbs-button-wrap'>
          <a href="<?php echo wp_nonce_url('?page=' . $zbs->slugs['module-activate-redirect'] . '&jpcrm-module-name=woo-sync', 'jpcrmmoduleactivateredirectnonce' ); ?>" class='btn btn-cta'><?php _e("Connect your WooCommerce Store to Jetpack CRM","zero-bs-crm"); ?></a>
        </div>
      </div>
    ...
    <div class="container margin-top30">
      ...
      <div class="feature-list block">
  				...
  				<div class="feature-block first">
  					<img alt="<?php _e('CRM Dashboard','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/crm-dash.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('CRM Dashboard','zero-bs-crm') ?></h5>
  					<p><?php _e('See at a glance the key areas of your CRM: e.g. Contact Activity, Contact Funnel, and Revenue snapshot.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block last">
  					<img alt="<?php _e('Limitless Contacts','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/customers.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Limitless Contacts','zero-bs-crm') ?></h5>
  					<p><?php _e('Add as many contacts as you like. No limits to the number of contacts you can add to your CRM.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block first">
  					<img alt="<?php _e('Quote Builder','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/quotes.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Quote Builder','zero-bs-crm') ?></h5>
  					<p><?php _e('Do you find yourself writing similar quotes/proposals over and over? Quote Builder makes it easy for your team.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block last">
  					<img alt="<?php _e('Invoicing','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/invoices.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Invoicing','zero-bs-crm') ?></h5>
  					<p><?php _e('Got clients or people to bill? Easily create invoices, and get paid online (pro). Clients can see all Invoices in one place on the Client Portal.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block first">
  					<img alt="<?php _e('Transactions','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/transactions.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Transactions','zero-bs-crm') ?></h5>
  					<p><?php _e('Log transactions against contacts or companies, and reconcile to invoices. Track payments, ecommerce data, and LTV (lifetime value).','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block last">
  					<img alt="<?php _e('B2B Mode','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/b2b.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('B2B Mode','zero-bs-crm') ?></h5>
  					<p><?php _e('Manage leads working at Companies? B2B mode lets you group contacts under a Company and keep track of sales easier.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block first">
  					<img alt="<?php _e('Automations','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/auto.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Automations','zero-bs-crm') ?><span class='pro'>Entrepreneur</span></h5>
  					<p><?php _e('Set up rule-based triggers and actions to automate your CRM work. Automatically Email new contacts, Distribute Leads, plus much more.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block last">
  					<img alt="<?php _e('Send SMS','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/sms.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Send SMS','zero-bs-crm') ?><span class='pro'>Entrepreneur</span></h5>
  					<p><?php _e('Want to get in front of your customers, wherever they are? Send SMS messages to your contacts from their CRM record.','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block first">
  					<img alt="<?php _e('Client Portal Pro','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/cpp.png', ZBS_ROOTFILE); ?>">
  					<h5><?php _e('Client Portal Pro','zero-bs-crm') ?><span class='pro'>Entrepreneur</span></h5>
  					<p><?php _e('Create a powerful client portal in one click! Easily share files with clients via their contact record. Tweak the portal to fit your branding, and more!','zero-bs-crm') ?></p>
  				</div>

  				<div class="feature-block last">
  					<img alt="<?php _e('Mail Campaigns','zero-bs-crm') ?>" src="<?php echo plugins_url('/i/mail.png', ZBS_ROOTFILE); ?>">
            <?php if($isv3){  ?>
  					  <h5><?php _e('Mail Campaigns','zero-bs-crm') ?><span class='pro-elite'>Elite</span></h5>
            <?php }else{ ?>
  					  <h5><?php _e('Mail Campaigns','zero-bs-crm') ?><span class='pro'>Entrepreneur</span></h5>
            <?php } ?>
  					<p><?php _e('Send Email Broadcasts and Sequences to your CRM contacts using our <strong>powerful</strong> Mail Campaigns v2.0. which is linked directly into your CRM data!','zero-bs-crm') ?></p>
  				</div>
      ...
      <div class="intro zbs-features">
        <div class="block" style="padding-bottom:0;">
						...
						<div class="testimonial-block">
							<img alt="Thumbnail of Michael Short" src="<?php echo plugins_url('/i/mb.jpg', ZBS_ROOTFILE); ?>">
              ...
              <p class='who'><strong>Michael Short</strong>
						...
						<div class="testimonial-block">
							<img alt="Thumbnail of Dave Scribner" src="<?php echo plugins_url('/i/scribner.png', ZBS_ROOTFILE); ?>">
       ...
       <div class="upgrade-cta upgrade">
          <div class="block">
             <div class="left">
                ...
                <ul>
                   <li style="width:100%;text-align:center;margin-bottom: 30px;">Access to 30+ Extensions:</li>
             ...
             <div class="right">
                ...
                <div class="price">
                   <span class="amount"><span class="dollar">$</span><?php echo ENTREPRENEUR_YEARLY_PRICE; ?></span><br>
                   <span class="term">per year</span>
                </div>
                <div class="zbs-button-wrap">
                   <a href="<?php echo $zbs->urls['upgrade']; ?>" rel="noopener noreferrer" target="_blank" class="upgrade-today btn btn-bta">
                   Upgrade your CRM today</a>
                </div>
             </div>
             <div class="clear"></div>
          </div> <!-- / .block -->
...
function zeroBSCRM_html_datatools(){
  ...
  if ( !$deleting_data ){ ?>
        ...
        <div id="zero-bs-tools" class="ui segment" style="margin-right:20px;">
          <h2 class="sbhomep"><?php _e( 'Welcome to Jetpack CRM Tools', 'zero-bs-crm' );?></h2>
          <p class="sbhomep"><?php _e( 'This is the home for all of the different admin tools for Jetpack CRM which import data, excluding the <strong>Sync</strong> Extensions.', 'zero-bs-crm' );?></p>
          <p class="sbhomep">
          ...
          <div class='mass-delete' style="text-align:center;">
              ...
              <div class="zbs-delete-box" style="max-width:70%;margin:auto;">
                <p class='ui message warning'>
                    <i class='ui icon exclamation'></i><b> <?php _e("Warning: This cannot be undone", "zero-bs-crm"); ?></b>
                </p>
...
function zeroBSCRM_html_installextensionshelper(){
  ...
  <?php
          ...
          ##/WLREMOVE
  ?>
          <h2><?php _e( 'Installing Extensions for Jetpack CRM', 'zero-bs-crm' );?></h2>
          <p class="intro"><?php echo sprintf( __( 'To control which modules are active, please go the <a href="%s">Core Module page</a>.', 'zero-bs-crm' ), admin_url( 'admin.php?page='.$zbs->slugs['modules'] ) );?></p>
          <p class="intro"><?php echo sprintf( __('To install premium extensions, purchased in a bundle or individually please go to <a href="%s">Plugins</a> and add your new extensions there.', 'zero-bs-crm'), admin_url( 'plugins.php' ) );?></p>
          <p class="intro-buttons">
            ...
            <?php ##WLREMOVE ?>
            <a href="<?php echo admin_url( 'admin.php?page='.$zbs->slugs['extensions'] ); ?>" class="ui button green"><i class="fa fa-search" aria-hidden="true"></i> <?php _e( 'Browse Extensions', 'zero-bs-crm' );?></a>
            <?php ##/WLREMOVE ?>
          </p>
    </div>
...
function zeroBSCRM_extension_installer_promo(){
  ...
  ?>
  <div class="bullie">
    <img src="<?php echo jpcrm_get_logo(false); ?>" alt="<?php _e('Jetpack CRM logo','zero-bs-crm') ?>">
  </div>
  <?php
}
  ...
  global $wpdb, $zbs;  #} Req
    ...
    if (zeroBSCRM_isWL()){  #WL - leave old ?>
        <div id="sbSubPage" style="width:600px"><h2 class="sbhomep"><?php _e("Feedback Makes the CRM better","zero-bs-crm");?></h2>
          <p class="sbhomep"><?php _e("We love to hear what you think of our CRM! Your feedback helps us make the CRM even better, even if you're hitting a wall with something (it's useful so long as it's constructive criticism!)","zero-bs-crm");?></p>
    ...
    <?php } else { #ZBS ?>
        <div id="sbSubPage" style="width:800px"><h2 class="sbhomep"><?php _e("Feedback Makes Jetpack CRM better","zero-bs-crm");?></h2>
          <p class="sbhomep"><?php _e("We love to hear what you think of Jetpack CRM! Your feedback helps us make the CRM even better, even if you're hitting a wall with something (it's useful if it's constructive criticism!)","zero-bs-crm");?></p>
...
function zeroBSCRM_html_extensions_forWelcomeWizard(){
  ...
  global $wpdb, $zbs;  #} Req ?>
        ...
        <div id="sbSubPage" style="width:100%;max-width:1000px"><h2 class="sbhomep"><?php _e("Power Up your CRM","zero-bs-crm");?></h2>
          <p class="sbhomep"><?php _e("We hope that you love using Jetpack CRM and that you agree with our mentality of stripping out useless features and keeping things simple. Cool.","zero-bs-crm");?></p>
          <p class="sbhomep"><?php _e("We offer a few extensions which supercharge your CRM. As is our principle, though, you wont find any bloated products here. These are simple, effective power ups for Jetpack CRM. And compared to pay-monthly costs, they're affordable! Win!","zero-bs-crm");?></p>
...
function zeroBSCRM_extensions_init_install(){
    ...
    if ( isset( $_GET['zbsinstall'] ) && ! empty( $_GET['zbsinstall'] ) ) {
      ...
      if (
        ...
        $installName = 'Unknown'; if (isset($extension_details['name'])) $installName = $extension_details['name'];
        ...
        wp_redirect( zbsLink( $zbs->slugs['modules'] ) );
    ...
    ##WLREMOVE
          ...
          echo '<div class="zbs-page-wrap thinner single-info-start">';
            ...
            echo '<div class="ui segment main-header-img">';
              ...
              echo '<div class="main-image full-size-image">';
                echo '<img src="' . $info->image .  '" alt="' . $info->name .  '"/>';
              echo '</div>';

              echo '<div class="below-main-image about-author-block">';
                  //start the about block
                  echo '<div class="about-img"><img alt="Jetpack CRM logo" src="' . $info->by . '"/>';
                    echo '<div class="top-info-block">';
                    echo '<h4 class="extension-name">' . $info->name . '</h4>';
                    echo '<div class="who">'. __('by ', 'zero-bs-crm') .'<a class="by-url" href="' . $zbs->urls['home']   .  '" target="_blank">Jetpack CRM</a></div>';            
                    echo '</div>';
                  echo '</div>';
                  //end the about block
                  ...
                  echo '<div class="actions-block"><div class="install-ext">';
                    if ( $installed ) {
                      ...
                      echo '<a href="' . esc_url( $zbs->urls['account'] ) . '" class="ui blue button" target="_blank"><i class="download icon"></i> ' . __( 'Download', 'zero-bs-crm' ) . '</a>';
                    } else {
            ...
            if(!$bundle){
              echo '<div class="bullie-wrap">';
                echo '<div class="bullie">';
                  echo '<img src="'. jpcrm_get_logo() . '" alt="Jetpack CRM" style="height:48px;padding:10px;">';
                  echo '<div class="upgrade">' . __('Purchase the Entrepreneur Bundle to get access to all of our CRM extensions.', 'zero-bs-crm') . '</div>';
                  echo '<a class = "ui button green mini upgrade-bullie-box" href="'. $zbs->urls['upgrade'].'" target = "_blank"><i class="cart plus icon"></i> ' . __('Start', 'zero-bs-crm') . '</a>';
                echo '</div>';
              echo '</div>';
          ...
          echo '<div class="zbs-page-wrap thinner" id="single-ext-desc">';
            echo '<div class="ui segment main-talk">';
              ...
              // buy
              if ( !$installed && !$bundle ) {
                echo '<hr /><div style="margin:2em;text-align:center"><a href="' . esc_url( $sales_link ) . '" class = "ui large blue button" target="_blank"><i class="cart icon"></i> ' . __( 'Buy Extension', 'zero-bs-crm' ) . '</a></div>';
              }
            echo '</div>';
          echo "</div>"; 
          //id="single-ext-desc"
    ...
    ##/WLREMOVE
        ...
        ##WLREMOVE
          ...
          echo '<div class="zbs-page-wrap">';
            ...
            echo '<div class="bullie-wrap">';
              echo '<div class="bullie">';
                echo '<img src="'. jpcrm_get_logo(false) . '" alt="Jetpack CRM" style="height: 48px; padding:10px;">';
                echo '<div class="upgrade">' . __('Purchase the Entrepreneur Bundle to get access to all of our CRM extensions.', 'zero-bs-crm') . '</div>';
                echo '<a class = "ui button green mini upgrade-bullie-box" href="'.$zbs->urls['upgrade'] .'" target = "_blank"><i class="cart plus icon"></i> ' . __('Buy  Now', 'zero-bs-crm') . '</a>';
              echo '</div>';
            echo '</div>';
            echo '<div class="clear"></div>';
            }  
            echo '<div class="ui top attached header premium-box"><h3 class="box-title">' . __('Premium Extensions', 'zero-bs-crm').'</h3>   <a class="guides ui button blue mini" href="'. $zbs->urls['docs']  .'" target="_blank"><i class="book icon"></i> '. __('Knowledge-base', 'zero-bs-crm')  .'</a> <a class="guides ui button blue basic mini" href="' . zeroBSCRM_getAdminURL( $zbs->slugs['modules'] ) . '"><i class="puzzle piece icon"></i> '. __('Core Modules', 'zero-bs-crm')  .'</a>   </div>';
            echo '<div class="clear"></div>';
              ...
              if (is_array($extensions->paid)) foreach($extensions->paid as $extension){
                ...
                if (!in_array($extension->id, $idsToHide)){
                    ...
                    echo "<div class='two wide column'>";
                      echo "<img alt='" . $extension->name . "' src='" . $extension->image  ."'/>";
                    echo "</div>";
                      ...
                      if($showLinkButton){
                        ...
                        if ($extkey == 'apiconnector'){
                            ...
                            // api connector
                              ...
                              // view
                              echo "<a href='". esc_url($zbs->urls['apiconnectorsales'])  ."' target='_blank'><span class='ui button orange mini'>" . __('View', 'zero-bs-crm') . "</span></a>";                      
                              
                              // download or buy
                              if ($bundle)
                                echo "<a href='". esc_url($zbs->urls['account'])  ."' target='_blank'><span class='ui button green mini'>" . __('Download', 'zero-bs-crm') . "</span></a>";
                              else
                                echo "<a href='". esc_url($sales_link)  ."' target='_blank'><span class='ui button green mini'>" . __('Buy', 'zero-bs-crm') . "</span></a>";

                          } else {

                            // non api connector
                            echo "<a href='". esc_url($more_url)  ."'><span class='ui button orange mini'>" . __('View', 'zero-bs-crm') . "</span></a>";
                            ...
                            if (!$installed){
                              ...
                              if (isset($extension->docs) && !empty($extension->docs)) echo "<a href='". esc_url($extension->docs)  ."' target='_blank'><span class='ui button blue mini'>" . __('Docs', 'zero-bs-crm') . "</span></a>";
                          }
                        echo '</div>';
                      }
                ...
                if ($e == 1){
                    ...
                    // End of row
                      ...
                      echo "<div class='two wide column'>";
                        echo "<img alt='" . __('Coming Soon','zero-bs-crm') . "' src='" . plugins_url('i/soon.png', ZBS_ROOTFILE)  ."'/>";
                      echo "</div>";

                      echo "<div class='six wide column ext-desc'>";
                        echo "<div class='title'>" . __('Coming soon', 'zero-bs-crm'). '</div>';
                        echo "<div class='content'>" . __('See and vote for what extensions we release next', 'zero-bs-crm')  . '</div>';
                  
                        echo '<div class="hover"></div>';
                        echo "<a class='hover-link' href='". esc_url($zbs->urls['soon'])  ."' target='_blank'><span class='ui button orange mini'>" . __('View', 'zero-bs-crm') . "</span></a>";
                      echo "</div>";
	...
	if ( !empty( $module_messages ) ) {
		...
		if ( $module_messages['success'] ) {
			...
			if ($module_messages['action'] == 'install' && $module_messages['extension_name'] == 'woosync'){
				...
				// assume no keys yet, tell em
				$msgHTML .= '<hr />' . __( 'To get started importing your WooCommerce orders, visit the WooSync hub:', 'zero-bs-crm' ) . '<p style="padding:1em"><a href="' . zbsLink( $zbs->slugs['woosync'] ) . '" class="ui button green">' . __( 'Get started with WooSync', 'zero-bs-crm' ) . '</a></p>';
              ...
              foreach(zeroBSCRM_extensions_free() as $k => $v){
                ...
                if (is_array($v)){
                    ...
                    echo "<div class='six wide column ext-desc'>";
                      ...
                      echo '<div class="hover"></div>';
                      echo "<a class='hover-link' href='". esc_url($modify_url)  ."'><span class='ui button ". $amend_color  ." mini'>" . $amend . "</span></a>";
    ...
    <div id="zbsNoAccessPage">
      <div id="zbsNoAccessMsgWrap">
        ...
        <div class="zbsNoAccessMsg">
          <h2><?php _e('Access Restricted', 'zero-bs-crm'); ?></h2>
          <p><?php _e('You do not have access to this '.$noAccessTypeStr.'.', 'zero-bs-crm'); ?></p>
        </div>
