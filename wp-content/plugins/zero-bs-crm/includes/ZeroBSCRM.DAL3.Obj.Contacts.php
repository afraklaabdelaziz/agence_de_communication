...
class zbsDAL_contacts extends zbsDAL_ObjectLayer {
    ...
    */
        ...
        if (
            ...
            #} ============= PRE-QUERY ============
                ...
                if ($withValues && !$onlyID){
                    ...
                    // invs:
                    $extraSelect .= ',(SELECT SUM(invstotal.zbsi_total) FROM '.$ZBSCRM_t['invoices'].' as invstotal WHERE invstotal.ID IN (SELECT DISTINCT zbsol_objid_from FROM '.$ZBSCRM_t['objlinks']." WHERE zbsol_objtype_from = ".ZBS_TYPE_INVOICE." AND zbsol_objtype_to = ".ZBS_TYPE_CONTACT." AND zbsol_objid_to = contact.ID)) as invoices_total";
                    // trans (with status):
                            ...
                            if (!isset($limitedFields) || !is_array($limitedFields) || $limitedFields == -1){
                                ...
                                if (isset($data['externalSources']) && is_array($data['externalSources']) && count($data['externalSources']) > 0) {
                                    ...
                                    foreach ($data['externalSources'] as $es){
                                        ...
                                        $esID = $this->DAL()->addUpdateExternalSource(array(
    ...
    public function deleteContact($args=array()){
        ...
        if (!empty($id) && $id > 0) {
            ...
            if ($saveOrphans === false){
                ...
                if ($zbs->isDAL2()) $this->DAL()->deleteTagObjLinks(array(
     ...
     * @return bool success
     */
    public function update_contact_email( $id=-1, $email_address ){
