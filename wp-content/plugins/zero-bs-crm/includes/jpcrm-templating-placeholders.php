...
class jpcrm_templating_placeholders {
	...
	public function build_placeholders( $include_custom_fields = true ){
		...
		if ( isset( $placeholders['quote']['title'] ) ) $placeholders['quote']['title']['aliases'] = array( '##QUOTETITLE##' );
				...
				foreach ( $object_model as $field_index => $field_info ){
					...
					if ( !in_array( $field_index, $excluded_slugs ) ){
						...
						if ( !isset( $placeholders[ $object_type_key ][ $new_key ] ) ){
							...
							if ( isset( $field_info['label'] ) ) $description = $object_label . ' ' . $field_info['label'];
							...
							// improved description where available
							if ( isset( $field_info['label'] ) ) $placeholders[ $object_type_key ][ $new_key ]['description'] = $object_label . ' ' . $field_info['label'];

							// trying to future proof, added a few helper attributes:
							if ( isset( $field_info['format'] ) ) $placeholders[ $object_type_key ][ $new_key ]['expected_format'] = $field_info['format'];
