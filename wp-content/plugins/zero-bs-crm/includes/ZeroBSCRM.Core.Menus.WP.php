...
function zeroBSCRM_menu_buildMenu(){
	...
	*/
	...
	if ($zbsMenuMode == ZBS_MENU_SLIM){
		...
		// Companies (sub)
		if ($b2bMode == 1) $menu['zbs']['subitems']['companies'] = array(
		...
		$menu['zbs']['subitems']['modules'] = array(
		...
		if ( current_user_can( 'manage_options' ) ) {
			$menu['zbs']['subitems']['installext'] = array(
	...
	// ===================================================
	if ($b2bMode == 1 && $zbsMenuMode != ZBS_MENU_SLIM){
	...
	if ( $useCalendar > 0 && $zbsMenuMode != ZBS_MENU_SLIM ) {
		...
		$menu['calendar']['subitems']['list'] = array(
									'title' => __('Task','zero-bs-crm').' '.__('List','zero-bs-crm'),
									'url' => $zbs->slugs['manage-events-list'],
									'perms' => 'admin_zerobs_customers',
									'order' => 1, 'wpposition' => 3,
	...
	// ===================================================	
		...
		// Check if it's in the context of WoA (Atomic infrastructure).
		if ( $zbsMenuMode != ZBS_MENU_SLIM && is_plugin_active( 'jetpack/jetpack.php' ) && class_exists( '\Automattic\Jetpack\Status\Host' ) && ( new \Automattic\Jetpack\Status\Host() )->is_woa_site() ) {
			$menu_position = 52;
		} else {
			$menu_position = 100;
		}
		...
		$menu['settings'] = array(
									...
									'url' => $zbs->slugs['home'],
									'perms' => 'admin_zerobs_manage_options',
									'order' => 100, 'wpposition' => $menu_position,
									'subitems' => array(),
									'callback' => 'zeroBSCRM_pages_home',
		...
		// CRM modules
		$menu['settings']['subitems']['modules'] = array(
			'title' => '<span>'.__('Core Modules','zero-bs-crm').'</span>',
			'url' => $zbs->slugs['modules'],
			'perms' => 'admin_zerobs_manage_options',
			'order' => 10,
