...
/* ======================================================
  ...
  function zeroBSCRM_isClientPortalPage(){
    #} zeroBSCRM_clientPortal_isPortalPage is in ZeroBSCRM.Portal.php but if Portal disabled that fileis not included
    #} see if(zeroBSCRM_isExtensionInstalled('portal') && !defined('ZBSCRM_INC_PORTAL')) require_once(ZEROBSCRM_PATH . 'includes/ZeroBSCRM.Portal.php');
    if(function_exists('zeroBSCRM_clientPortal_isPortalPage')){
      if (zeroBSCRM_clientPortal_isPortalPage()) return true;
    }
    return false;
  }
  ...
  function zeroBSCRM_isAnyContactPage(){
    ...
    if (zeroBSCRM_isContactSearchPage()) return true;
  ...
  // WH quickly made - for mikes export page
  function zeroBSCRM_isContactSearchPage(){
    global $zbs;
    return zeroBS_isPage(array('admin.php'),false,array($zbs->slugs['customer-search']));
  }
