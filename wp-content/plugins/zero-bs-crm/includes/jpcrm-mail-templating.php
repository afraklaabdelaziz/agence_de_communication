  		...
  		if ( current_user_can( 'admin_zerobs_manage_options' ) ) {  
			...
			if (isset( $_GET['template_id'] ) && !empty( $_GET['template_id'] ) ){
				...
				echo $placeholder_templating->replace_placeholders( array(  'global', 'invoice' ), $html, array(
						...
						'title' 			=> __('Template Preview','zero-bs-crm'),
						'msg-content'		=> $message_content,
						'unsub-line'		=> $unsub_line,
	    ...
	    if (!empty($quoteID)){
	        ...
	        if (!empty($templatedHTML)){
        		...
        		if ($hasQuote){
					...
					$bodyHTML = $placeholder_templating->replace_placeholders( array(  'global', 'quote' ), $bodyHTML, $replacements, array( ZBS_TYPE_QUOTE => $quote ) );
		            ...
		            if (isset($zbs_biz_name) && !empty($zbs_biz_name)) $unsub_line .= ' by '.$zbs_biz_name;
	        ...
	        if (!empty($templatedHTML)){
	        	...
	        	$bodyHTML = '';
	        		...
	        		if ($hasQuote){
                		...
                		$quote_url = zbsLink('edit',$quoteID,'zerobs_quote');
                		$quote_edit_url = zbsLink('edit',$quoteID,'zerobs_quote'); 
						...
						// build msg-content html
						$bodyHTML = $placeholder_templating->replace_placeholders( array(  'global', 'quote' ), $bodyHTML, $replacements );
...
function zeroBSCRM_invoice_generateNotificationHTML( $invoiceID=-1, $return=true ){
  ...
  if (!empty($invoiceID) && $invoiceID > 0){
    ...
    if ( !empty( $html ) ){
      ...
      // replacements
      $html = $placeholder_templating->replace_placeholders( array(  'global', 'invoice' ), $html, $replacements, array( ZBS_TYPE_INVOICE => zeroBS_getInvoice( $invoiceID ) ) );
...
function zeroBSCRM_statement_generateNotificationHTML($contactID=-1,$return=true){
	    ...
	    if (!empty($contactID)){
	        ...
	        if (!empty($html)){
	            ...
	            // replacements
	            $html = $placeholder_templating->replace_placeholders( array(  'global', 'invoice' ), $html, $replacements );
...
function zeroBSCRM_Portal_generateNotificationHTML( $pwd = -1, $return = true, $email = null, $contact_id = false ) {
	    ...
	    if ( ! empty( $pwd ) ) {
	        ...
	        if (!empty($templatedHTML)){
	            ...
	            // replacements
	            $html = $placeholder_templating->replace_placeholders( array(  'global' ), $html, $replacements, $replacement_objects );
