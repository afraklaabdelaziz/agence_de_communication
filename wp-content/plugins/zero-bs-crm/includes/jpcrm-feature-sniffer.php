...
class JPCRM_FeatureSniffer {
  ...
  public function sniff_for_plugin( $args=array(), $is_silent=false ) {
    ...
    if ( !$is_silent && !$is_dismissed && current_user_can( 'activate_plugins' ) ) {
      ...
      $message_template = __( 'Your CRM has an integration with your <code>%s</code> plugin, but it is not currently enabled. Interested? Learn more by going <a href="%s" target="_blank">here</a>.' );
      $this->alerts[] = array(
  ...
  public function dismiss_alert( $feature_slug ) {
    ...
    if ( !empty( $feature_slug ) ){
      ...
      update_option( 'jpcrm_hide_'.$args['feature_slug'].'_feature_alert', 1, false );
      return true;
